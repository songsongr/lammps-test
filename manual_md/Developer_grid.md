...
:::::::::::::::::::::::::::::::::::::::::::::::: {#use-of-distributed-grids-within-style-classes .section}
# [4.26. ]{.section-number}Use of distributed grids within style classes[](#use-of-distributed-grids-within-style-classes "Link to this heading"){.headerlink}

::: versionadded
[Added in version 22Dec2022.]{.versionmodified .added}
:::

The LAMMPS source code includes two classes which facilitate the creation and use of distributed grids. These are the Grid2d and Grid3d classes in the src/grid2d.cpp.h and src/grid3d.cpp.h files respectively. As the names imply, they are used for 2d or 3d simulations, as defined by the [[dimension]{.doc}]dimension.md){.reference .internal} command.

The [[Howto_grid]{.doc}]Howto_grid.md){.reference .internal} page gives an overview of how distributed grids are defined from a user perspective, lists LAMMPS commands which use them, and explains how grid cell data is referenced from an input script. Please read that page first as it motivates the coding details discussed here.
...
::: {#style-commands .section}
## [4.26.1. ]{.section-number}Style commands[](#style-commands "Link to this heading"){.headerlink}

Style commands which can define and use distributed grids include the [[compute]{.doc}]compute.md){.reference .internal}, [[fix]{.doc}]fix.md){.reference .internal}, [[pair]{.doc}]pair_style.md){.reference .internal}, and [[kspace]{.doc}]kspace_style.md){.reference .internal} styles. If you wish grid cell data to persist across timesteps, then use a fix. If you wish grid cell data to be accessible by other commands, then use a fix or compute. Currently in LAMMPS, the [[pair_style amoeba]{.doc}]pair_amoeba.md){.reference .internal}, [[kspace_style pppm]{.doc}]kspace_style.md){.reference .internal}, and [[kspace_style msm]{.doc}]kspace_style.md){.reference .internal} commands use distributed grids but do not require either of these capabilities; they thus create and use distributed grids internally. Note that a pair style which needs grid cell data to persist could be coded to work in tandem with a fix style which provides that capability.
...
::::::: {#grid-data-allocation-and-access .section}
## [4.26.2. ]{.section-number}Grid data allocation and access[](#grid-data-allocation-and-access "Link to this heading"){.headerlink}
...
::::: {#grid-class-constructors .section}
## [4.26.3. ]{.section-number}Grid class constructors[](#grid-class-constructors "Link to this heading"){.headerlink}
...
Both constructors take the LAMMPS instance pointer and a communicator over which the grid will be distributed. Typically this is the *world* communicator the LAMMPS instance is using. The [[kspace_style msm]{.doc}]kspace_style.md){.reference .internal} command creates a series of grids, each of different size, which are partitioned across different sub-communicators of processors. Both constructors are also passed the global grid size: *gnx* by *gny* by *gnz*.
...
::::: {#grid-class-set-methods .section}
## [4.26.4. ]{.section-number}Grid class set methods[](#grid-class-set-methods "Link to this heading"){.headerlink}
...
Also note that this method must be invoked after any *set\_\*()* methods have been used, since they can influence the assignment of owned and ghost cells.
...
::::: {#more-grid-class-set-methods .section}
## [4.26.6. ]{.section-number}More grid class set methods[](#more-grid-class-set-methods "Link to this heading"){.headerlink}

The following 2 methods can be used to override settings made by the constructors above. If used, they must be called called before the *setup_comm()* method is invoked, since it uses the settings that these methods override. In LAMMPS these methods are called by by the [[kspace_style msm]{.doc}]kspace_style.md){.reference .internal} command for the grids it instantiates using the 2nd constructor above.
...
::::: {#grid-class-get-methods .section}
## [4.26.7. ]{.section-number}Grid class get methods[](#grid-class-get-methods "Link to this heading"){.headerlink}
...
::::::::::: {#grid-class-owned-ghost-communication .section}
## [4.26.8. ]{.section-number}Grid class owned/ghost communication[](#grid-class-owned-ghost-communication "Link to this heading"){.headerlink}
...
This can be checked by callers who have the option to change the global grid size to ensure more efficient nearest-neighbor-only communication if they wish. In this case, they instantiate a grid of a given size (resolution), then invoke *setup_comm()* followed by *ghost_adjacent()*. If the ghost cells are not adjacent, they destroy the grid instance and start over with a higher-resolution grid. Several of the [[kspace_style pppm]{.doc}]kspace_style.md){.reference .internal} command variants have this option.
:::::::::::

------------------------------------------------------------------------

::::::: {#grid-class-remap-methods-for-load-balancing .section}
## [4.26.9. ]{.section-number}Grid class remap methods for load balancing[](#grid-class-remap-methods-for-load-balancing "Link to this heading"){.headerlink}

The following methods are used when a load-balancing operation, triggered by the [[balance]{.doc}]balance.md){.reference .internal} or [[fix balance]{.doc}]fix_balance.md){.reference .internal} commands, changes the partitioning of the simulation domain into processor subdomains.
...
::::::: {#grid-class-i-o-methods .section}
## [4.26.10. ]{.section-number}Grid class I/O methods[](#grid-class-i-o-methods "Link to this heading"){.headerlink}
...
::::: {#style-class-grid-access-methods .section}
## [4.26.11. ]{.section-number}Style class grid access methods[](#style-class-grid-access-methods "Link to this heading"){.headerlink}

A style command can enable its grid cell data to be accessible from other commands. For example [[fix ave/grid]{.doc}]fix_ave_grid.md){.reference .internal} or [[dump grid]{.doc}]dump.md){.reference .internal} or [[dump grid/vtk]{.doc}]dump.md){.reference .internal}. Those commands access the grid cell data by using a *grid reference* in their input script syntax, as described on the [[Howto_grid]{.doc}]Howto_grid.md){.reference .internal} doc page. They look like this:
...
::: {#final-notes .section}
## [4.26.12. ]{.section-number}Final notes[](#final-notes "Link to this heading"){.headerlink}
