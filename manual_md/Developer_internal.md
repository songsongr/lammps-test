...
::::: {#deprecated-styles .section}
## [4.25.1. ]{.section-number}DEPRECATED Styles[](#deprecated-styles "Link to this heading"){.headerlink}
...
There is also a section in the user's guide for [[removed commands and packages]{.doc}]Commands_removed.md){.reference .internal} with additional explanations.
:::::

::::::::::::: {#internal-fix-styles .section}
## [4.25.2. ]{.section-number}Internal fix styles[](#internal-fix-styles "Link to this heading"){.headerlink}

These provide an implementation of features that would otherwise have been replicated across multiple styles. The used fix ID is generally derived from the compute or fix ID creating the fix with some string appended. When needed, the fix can be looked up with [`Modify::get_fix_by_id()`{.docutils .literal .notranslate}]{.pre}, which returns a pointer to the fix instance. The data managed by the fix can be accessed just as for other fixes that can be used in input files.

::: {#fix-dummy .section}
### fix DUMMY[](#fix-dummy "Link to this heading"){.headerlink}

Most fix classes cannot be instantiated before the simulation box has been created since they access data that is only available then. However, in some cases it is required that a fix must be at or close to the top of the list of all fixes. In those cases an instance of the DUMMY fix style may be created by calling [`Modify::add_fix()`{.docutils .literal .notranslate}]{.pre} and then later replaced by the intended fix through calling [`Modify::replace_fix()`{.docutils .literal .notranslate}]{.pre}.
:::

::::: {#fix-store-atom .section}
### fix STORE/ATOM[](#fix-store-atom "Link to this heading"){.headerlink}
...
- ID, group-ID are documented in [[fix]{.doc}]fix.md){.reference .internal} command, group-ID is *ignored*
...
Similar functionality is also available through using custom per-atom properties with [[fix property/atom]{.doc}]fix_property_atom.md){.reference .internal}. The choice between the two fixes should be based on whether the user should be able to access this per-atom data: if yes, then fix property/atom is preferred, otherwise fix STORE/ATOM.
:::::

::::: {#fix-store-global .section}
### fix STORE/GLOBAL[](#fix-store-global "Link to this heading"){.headerlink}
...
::::: {#fix-store-local .section}
### fix STORE/LOCAL[](#fix-store-local "Link to this heading"){.headerlink}
