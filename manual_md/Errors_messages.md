...
# [5.6. ]{.section-number}Error messages[](#error-messages "Link to this heading"){.headerlink}
...
Please also see the page with [[Warning messages]{.doc}]Errors_warnings.md){.reference .internal}.
...
:   The neighbor lists used by fix orient/fcc are messed up. If this error occurs, it is likely a bug, so send an email to the [developers](https://www.lammps.org/authors.html){.reference .external}.
...
:   The limit on atom IDs is set by the SMALLBIG, BIGBIG setting in your LAMMPS build. See the [[Build settings]{.doc}]Build_settings.md){.reference .internal} page for more info.
