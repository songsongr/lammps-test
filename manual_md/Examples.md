...
# [11. ]{.section-number}Example scripts[](#example-scripts "Link to this heading"){.headerlink}
...
If you uncomment the [[dump]{.doc}]dump.md){.reference .internal} command in the input script, a text dump file will be produced, which can be animated by various [visualization programs](https://www.lammps.org/viz.html){.reference .external}.

If you uncomment the [[dump image]{.doc}]dump.md){.reference .internal} command in the input script, and assuming you have built LAMMPS with a JPG library, JPG snapshot images will be produced when the simulation runs. They can be quickly post-processed into a movie using commands described on the [[dump image]{.doc}]dump_image.md){.reference .internal} doc page.

Animations of many of the examples can be viewed on the Movies section of the [LAMMPS website](https://www.lammps.org/movies.html){.reference .external}.
...
::::::: {#lowercase-directories .section}
## [11.1. ]{.section-number}Lowercase directories[](#lowercase-directories "Link to this heading"){.headerlink}
  ...
  kim           use of potentials from the [OpenKIM Repository](https://openkim.org){.reference .external}
...
Running the simulation produces the files *dump.indent* and *log.lammps*. You can visualize the dump file of snapshots with a variety of third-party tools highlighted on the [Visualization](https://www.lammps.org/viz.html){.reference .external} page of the LAMMPS website.

If you uncomment the [[dump image]{.doc}]dump_image.md){.reference .internal} line(s) in the input script a series of JPG images will be produced by the run (assuming you built LAMMPS with JPG support; see the [[Build_settings]{.doc}]Build_settings.md){.reference .internal} page for details). These can be viewed individually or turned into a movie or animated by tools like ImageMagick or QuickTime or various Windows-based tools. See the [[dump image]{.doc}]dump_image.md){.reference .internal} page for more details. E.g. this Imagemagick command would create a GIF file suitable for viewing in a browser.
...
::: {#uppercase-directories .section}
## [11.2. ]{.section-number}Uppercase directories[](#uppercase-directories "Link to this heading"){.headerlink}
...
The PACKAGES directory has a large number of subdirectories which correspond by name to specific packages. They contain scripts that illustrate how to use the command(s) provided in those packages. Many of the subdirectories have their own README files which give further instructions. See the [[Packages_details]{.doc}]Packages_details.md){.reference .internal} doc page for more info on specific packages.
:::
:::::::::
::::::::::
:::::::::::
