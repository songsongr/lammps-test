...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::: {#tutorial-for-thermalized-drude-oscillators-in-lammps .section}
# [10.5.8. ]{.section-number}Tutorial for Thermalized Drude oscillators in LAMMPS[](#tutorial-for-thermalized-drude-oscillators-in-lammps "Link to this heading"){.headerlink}

This Howto document explains how to use Drude oscillators in LAMMPS to simulate polarizable systems using the DRUDE package. As an illustration, the input files for a simulation of 250 phenol molecules are documented. First of all, LAMMPS has to be compiled with the DRUDE package included. Then, the data file and input scripts have to be modified to include the Drude dipoles and how to handle them.

Example input scripts are available: [`examples/PACKAGES/drude`{.docutils .literal .notranslate}]{.pre}
...
The sum of the masses of a core-Drude pair should be the mass of the initial (unsplit) atom, [\\(m_C + m_D = m\\)]{.math .notranslate .nohighlight}. The sum of their charges should be the charge of the initial (unsplit) atom, [\\(q_C + q_D = q\\)]{.math .notranslate .nohighlight}. A harmonic potential between the core and Drude partners should be present, with force constant [\\(k_D\\)]{.math .notranslate .nohighlight} and an equilibrium distance of zero. The (half-)stiffness of the [[harmonic bond]{.doc}]bond_harmonic.md){.reference .internal} [\\(K_D = k_D/2\\)]{.math .notranslate .nohighlight} and the Drude charge [\\(q_D\\)]{.math .notranslate .nohighlight} are related to the atom polarizability [\\(\\alpha\\)]{.math .notranslate .nohighlight} by
...
- [[Lamoureux and Roux]{.std .std-ref}](#lamoureux2){.reference .internal} suggest adopting a global half-stiffness, [\\(K_D\\)]{.math .notranslate .nohighlight} = 500 kcal/(mol Ang [\\({}\^2\\)]{.math .notranslate .nohighlight}) - which corresponds to a force constant [\\(k_D\\)]{.math .notranslate .nohighlight} = 4184 kJ/(mol Ang [\\({}\^2\\)]{.math .notranslate .nohighlight}) - for all types of core-Drude bond, a global mass [\\(m_D\\)]{.math .notranslate .nohighlight} = 0.4 g/mol (or u) for all types of Drude particles, and to calculate the Drude charges for individual atom types from the atom polarizabilities using equation (1). This choice is followed in the polarizable CHARMM force field.

- Alternately, [[Schroeder and Steinhauser]{.std .std-ref}](#schroeder){.reference .internal} suggest adopting a global charge [\\(q_D\\)]{.math .notranslate .nohighlight} = -1.0e and a global mass [\\(m_D\\)]{.math .notranslate .nohighlight} = 0.1 g/mol (or u) for all Drude particles, and to calculate the force constant for each type of core-Drude bond from equation (1). The timesteps used by these authors are between 0.5 and 2 fs, with the degrees of freedom of the Drude oscillators kept cold at 1 K.

- In both these force fields hydrogen atoms are treated as non-polarizable.

The motion of the Drude particles can be calculated by minimizing the energy of the induced dipoles at each timestep, by an iterative, self-consistent procedure. The Drude particles can be massless and therefore do not contribute to the kinetic energy. However, the relaxed method is computationally slow. An extended-lagrangian method can be used to calculate the positions of the Drude particles, but this requires them to have mass. It is important in this case to decouple the degrees of freedom associated with the Drude oscillators from those of the normal atoms. Thermalizing the Drude dipoles at temperatures comparable to the rest of the simulation leads to several problems (kinetic energy transfer, very short timestep, etc.), which can be remedied by the "cold Drude" technique ([[Lamoureux and Roux]{.std .std-ref}](#lamoureux2){.reference .internal}).
...
By recognizing the fix *drude*, LAMMPS will find and store matching DC-DP pairs and will treat DP as equivalent to their DC in the *special bonds* relations. It may be necessary to extend the space for storing such special relations. In this case extra space should be reserved by using the *extra/special/per/atom* keyword of either the [[read_data]{.doc}]read_data.md){.reference .internal} or [[create_box]{.doc}]create_box.md){.reference .internal} command. With our phenol, there is 1 more special neighbor for which space is required. Otherwise LAMMPS crashes and gives the required value.
...
As compared to the non-polarizable input file, *pair_coeff* lines need to be added for the DPs. Since the DPs have no Lennard-Jones interactions, their [\\(\\epsilon\\)]{.math .notranslate .nohighlight} is 0. so the only *pair_coeff* line that needs to be added is
...
Now for the thermalization, the simplest choice is to use the [[fix langevin/drude]{.doc}]fix_langevin_drude.md){.reference .internal}.
...
You will notice that the global temperature *thermo_temp* computed by LAMMPS is not 300. K as wanted. This is because LAMMPS treats DPs as standard atoms in its default compute. If you want to output the temperatures of the DC-DP pair centers of mass and of the DPs relative to their DCs, you should use the [[compute temp_drude]{.doc}]compute_temp_drude.md){.reference .internal}
...
This tells LAMMPS that we are using two pair_styles. The first one is as above (*lj/cut/coul/long 10.0*). The second one is a *thole* pair_style with default screening factor 2.6 ([[Noskov]{.std .std-ref}](#noskov2){.reference .internal}) and cutoff 10.0.
...
It is also possible to use a Nose-Hoover instead of a Langevin thermostat. This requires to use [[\*fix drude/transform\*]{.doc}]fix_drude_transform.md){.reference .internal} just before and after the time integration fixes. The *fix drude/transform/direct* converts the atomic masses, positions, velocities and forces into a reduced representation, where the DCs transform into the centers of mass of the DC-DP pairs and the DPs transform into their relative position with respect to their DC. The *fix drude/transform/inverse* performs the reverse transformation. For a NVT simulation, with the DCs and atoms at 300 K and the DPs at 1 K relative to their DC one would use
