...
:::: {#using-lammps-with-the-mdi-library-for-code-coupling .section}
# [10.1.9. ]{.section-number}Using LAMMPS with the MDI library for code coupling[](#using-lammps-with-the-mdi-library-for-code-coupling "Link to this heading"){.headerlink}

Client/server coupling of two (or more) codes is where one code is the "client" and sends request messages (data) to one (or more) "server" code(s). A server responds to each request with a reply message (data). This enables two (or more) codes to work in tandem to perform a simulation. In this context, LAMMPS can act as either a client or server code. It does this by using the [MolSSI Driver Interface (MDI) library](https://molssi-mdi.github.io/MDI_Library/){.reference .external}, developed by the [Molecular Sciences Software Institute (MolSSI)](https://molssi.org){.reference .external}, which is supported by the [[MDI]{.std .std-ref}]Packages_details.md#pkg-mdi){.reference .internal} package.

Alternate methods for coupling codes with LAMMPS are described on the [[Coupling LAMMPS to other codes]{.doc}]Howto_couple.md){.reference .internal} page.
...
The [[MDI]{.std .std-ref}]Packages_details.md#pkg-mdi){.reference .internal} package provides a [[mdi engine]{.doc}]mdi.md){.reference .internal} command, which enables LAMMPS to operate as an MDI engine. Its doc page explains the variety of standard and custom MDI commands which the LAMMPS engine recognizes and can respond to.

The package also provides a [[mdi plugin]{.doc}]mdi.md){.reference .internal} command, which enables LAMMPS to operate as an MDI driver and load an MDI engine as a plugin library.

The package furthermore includes a [[fix mdi/qm]{.doc}]fix_mdi_qm.md){.reference .internal} command, in which LAMMPS operates as an MDI driver in conjunction with a quantum mechanics code as an MDI engine. The post_force() method of the [`fix_mdi_qm.cpp`{.docutils .literal .notranslate}]{.pre} file shows how a driver issues MDI commands to another code. This command can be used to couple to an MDI engine, which is either a stand-alone code or a plugin library.

As explained in the [[fix mdi/qm]{.doc}]fix_mdi_qm.md){.reference .internal} command documentation, it can be used to perform *ab initio* MD simulations or energy minimizations, or to evaluate the quantum energy and forces for a series of independent systems. The [`examples/mdi`{.docutils .literal .notranslate}]{.pre} directory has example input scripts for all of these use cases.

The package also has a [[fix mdi/qmmm]{.doc}]fix_mdi_qmmm.md){.reference .internal} command in which LAMMPS operates as an MDI driver in conjunction with a quantum mechanics code as an MDI engine to perform QM/MM simulations. The LAMMPS input script partitions the system into QM and MM (molecular mechanics) atoms. As described below the [`examples/QUANTUM`{.docutils .literal .notranslate}]{.pre} directory has examples for coupling to 3 different quantum codes in this manner.

------------------------------------------------------------------------

The [`examples/mdi`{.docutils .literal .notranslate}]{.pre} directory contains Python scripts and LAMMPS input script which use LAMMPS as either an MDI driver or engine, or both. Currently, 5 example use cases are provided:
...
The [`examples/mdi/Run.sh`{.docutils .literal .notranslate}]{.pre} file illustrates how to launch both driver and engine codes so that they communicate using the MDI library via either MPI or sockets, or using the engine as a stand-alone code, or as a plugin library.

------------------------------------------------------------------------

As of March 2023, these are quantum codes with MDI support provided via Python wrapper scripts included in the LAMMPS distribution. These can be used with the fix mdi/qm and fix mdi/qmmm commands to perform QM calculations of an entire system (e.g. AIMD) or QM/MM simulations. See the [`examples/QUANTUM`{.docutils .literal .notranslate}]{.pre} sub-directories for more details:
...
There are also at least two quantum codes which have direct MDI support, [Quantum ESPRESSO (QE)](https://www.quantum-espresso.org/){.reference .external} and [INQ](https://gitlab.com/npneq/inq){.reference .external}. There are also several QM codes which have indirect support through QCEngine or i-PI. The former means they require a wrapper program (QCEngine) with MDI support which writes/read files to pass data to the quantum code itself. The list of QCEngine-supported and i-PI-supported quantum codes is on the [MDI webpage](https://molssi-mdi.github.io/MDI_Library/user_guide/mdi_ecosystem/){.reference .external}.
