...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: {#output-structured-data-from-lammps .section}
# [10.3.9. ]{.section-number}Output structured data from LAMMPS[](#output-structured-data-from-lammps "Link to this heading"){.headerlink}

LAMMPS can output structured data with the [[print]{.doc}]print.md){.reference .internal} and [[fix print]{.doc}]fix_print.md){.reference .internal} command. This gives you flexibility since you can build custom data formats that contain system properties, thermo data, and variables values. This output can be directed to the screen and/or to a file for post processing.

::::::::::::::::: {#writing-the-current-system-state-thermo-data-variable-values .section}
## Writing the current system state, thermo data, variable values[](#writing-the-current-system-state-thermo-data-variable-values "Link to this heading"){.headerlink}

Use the [[print]{.doc}]print.md){.reference .internal} command to output the current system state, which can include system properties, thermo data and variable values.
...
::: code-block-caption
[current_state.yaml]{.caption-text}[](#id2 "Link to this code"){.headerlink}
:::
...
::::::::: {#json .section}
### JSON[](#json "Link to this heading"){.headerlink}
...
::: code-block-caption
[current_state.json]{.caption-text}[](#id3 "Link to this code"){.headerlink}
:::
...
:::::::::::::::::::::::::: {#yaml-format-thermo-style-or-dump-style-output .section}
## YAML format thermo_style or dump_style output[](#yaml-format-thermo-style-or-dump-style-output "Link to this heading"){.headerlink}

:::::::::: {#extracting-data-from-log-file .section}
### Extracting data from log file[](#extracting-data-from-log-file "Link to this heading"){.headerlink}

::: versionadded
[Added in version 24Mar2022.]{.versionmodified .added}
:::

LAMMPS supports the thermo style "yaml" and for "custom" style thermodynamic output the format can be changed to YAML with [[thermo_modify line yaml]{.doc}]thermo_modify.md){.reference .internal}. This will produce a block of output in a compact YAML format - one "document" per run - of the following style:
...
:::::::: {#extracting-data-from-dump-file .section}
### Extracting data from dump file[](#extracting-data-from-dump-file "Link to this heading"){.headerlink}

::: versionadded
[Added in version 4May2022.]{.versionmodified .added}
:::

YAML format output has been added to multiple commands in LAMMPS, for example [[dump yaml]{.doc}]dump.md){.reference .internal} or [[fix ave/time]{.doc}]fix_ave_time.md){.reference .internal} Depending on the kind of data being written, organization of the data or the specific syntax used may change, but the principles are very similar and all files should be readable with a suitable YAML parser. A simple example for this is given below:
...
::::: {#processing-scalar-data-with-python .section}
### Processing scalar data with Python[](#processing-scalar-data-with-python "Link to this heading"){.headerlink}

<figure class="align-right" style="width: 33%">
<img src="_images/thermo_bondeng.png" alt="_images/thermo_bondeng.png" />
</figure>

After reading and parsing the YAML format data, it can be easily imported for further processing and visualization with the [pandas](https://pandas.pydata.org/){.reference .external} and [matplotlib](https://matplotlib.org/){.reference .external} Python modules. Because of the organization of the data in the YAML format thermo output, it needs to be told to process only the 'data' part of the imported data to create a pandas data frame, and one needs to set the column names from the 'keywords' entry. The following Python script code example demonstrates this, and creates the image shown on the right of a simple plot of various bonded energy contributions versus the timestep from a run of the 'peptide' example input after changing the [[thermo style]{.doc}]thermo_style.md){.reference .internal} to 'yaml'. The properties to be used for x and y values can be conveniently selected through the keywords. Please note that those keywords can be changed to custom strings with the [[thermo_modify colname]{.doc}]thermo_modify.md){.reference .internal} command.
...
::::: {#processing-scalar-data-with-perl .section}
### Processing scalar data with Perl[](#processing-scalar-data-with-perl "Link to this heading"){.headerlink}
...
::::::::::::::::::::::::: {#writing-continuous-data-during-a-simulation .section}
## Writing continuous data during a simulation[](#writing-continuous-data-during-a-simulation "Link to this heading"){.headerlink}

The [[fix print]{.doc}]fix_print.md){.reference .internal} command allows you to output an arbitrary string at defined times during a simulation run.

::::::::::::: {#id1 .section}
### YAML[](#id1 "Link to this heading"){.headerlink}
...
::: code-block-caption
[output.yaml]{.caption-text}[](#id4 "Link to this code"){.headerlink}
:::
...
::::::::::::: {#line-delimited-json-ld-json .section}
### Line Delimited JSON (LD-JSON)[](#line-delimited-json-ld-json "Link to this heading"){.headerlink}
...
::: code-block-caption
[output.json]{.caption-text}[](#id5 "Link to this code"){.headerlink}
:::
