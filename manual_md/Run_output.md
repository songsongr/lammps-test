...
::::::: {#screen-and-logfile-output .section}
# [4.3. ]{.section-number}Screen and logfile output[](#screen-and-logfile-output "Link to this heading"){.headerlink}
...
The last section gives aggregate statistics (across all processors) for pairwise neighbors and special neighbors that LAMMPS keeps track of (see the [[special_bonds]{.doc}]special_bonds.md){.reference .internal} command). This section will not always contain data, for example when there has not been a neighbor rebuild, or the neighbor list was constructed on the GPU or when a hybrid pair style was used and LAMMPS cannot determine a suitable (base) neighbor list to draw the statistics from.

The number of times neighbor lists were rebuilt is tallied, as is the number of potentially *dangerous* rebuilds. If atom movement triggered neighbor list rebuilding (see the [[neigh_modify]{.doc}]neigh_modify.md){.reference .internal} command), then dangerous reneighborings are those that were triggered on the first timestep atom movement was checked for. If this count is non-zero you may wish to reduce the delay factor to ensure no force interactions are missed by atoms moving beyond the neighbor skin distance before a rebuild takes place.

------------------------------------------------------------------------

If an energy minimization was performed via the [[minimize]{.doc}]minimize.md){.reference .internal} command, additional information is printed, e.g.
...
::::::::::::::: {#error-message-output .section}
# [4.4. ]{.section-number}Error message output[](#error-message-output "Link to this heading"){.headerlink}
...
## [4.4.1. ]{.section-number}A single line[](#a-single-line "Link to this heading"){.headerlink}
...
::::: {#two-lines .section}
## [4.4.2. ]{.section-number}Two lines[](#two-lines "Link to this heading"){.headerlink}
...
::::: {#three-lines .section}
## [4.4.3. ]{.section-number}Three lines[](#three-lines "Link to this heading"){.headerlink}
...
::::: {#four-lines .section}
## [4.4.4. ]{.section-number}Four lines[](#four-lines "Link to this heading"){.headerlink}

The three line output is expanded to four lines, if the input is modified through input pre-processing, e.g. when substituting variables. Now the last command is printed once in the original form and a second time after substitutions are applied. The caret character markers '\^' are applied to the second version. Example:
