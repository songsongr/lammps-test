...
::::::::: {#running-lammps-on-windows .section}
# [4.6. ]{.section-number}Running LAMMPS on Windows[](#running-lammps-on-windows "Link to this heading"){.headerlink}

To run a serial (non-MPI) executable, follow these steps:

- Install a LAMMPS installer package from [https://packages.lammps.org/windows.html](https://packages.lammps.org/windows.html){.reference .external}
...
- At the command prompt, type "lmp -in in.file.lmp", where [`in.file.lmp`{.docutils .literal .notranslate}]{.pre} is the name of your LAMMPS input script.
...
Alternately, you can also install a package with LAMMPS-GUI included and open the LAMMPS-GUI app (the package includes the command-line version of LAMMPS as well) and open the input file in the GUI and run it from there. For details on LAMMPS-GUI, see [https://lammps-gui.lammps.org/](https://lammps-gui.lammps.org/){.reference .external}
...
Download and install the MS-MPI runtime package [`msmpisetup.exe`{.docutils .literal .notranslate}]{.pre} from [https://www.microsoft.com/en-us/download/details.aspx?id=105289](https://www.microsoft.com/en-us/download/details.aspx?id=105289){.reference .external} (Note that the [`msmpisdk.msi`{.docutils .literal .notranslate}]{.pre} is **only** required for **compilation** of LAMMPS from source on Windows using Microsoft Visual Studio). After installation of MS-MPI perform a reboot.
...
where [`in.file.lmp`{.docutils .literal .notranslate}]{.pre} is the name of your LAMMPS input script. For the latter case, you may be prompted to enter the password that you set during installation of the MPI library software.
...
MPI parallelization will work for *all* functionality in LAMMPS and in many cases the MPI parallelization is more efficient than multi-threading since LAMMPS was designed from the ground up for MPI parallelization using domain decomposition. Multi-threading is only available for selected styles and implemented on top of the MPI parallelization. Multi-threading is most useful for systems with large load imbalances when using domain decomposition and a smaller number of threads (\<= 8).
:::::::::
::::::::::
:::::::::::
