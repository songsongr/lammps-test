...
# [7.2. ]{.section-number}Measuring performance[](#measuring-performance "Link to this heading"){.headerlink}

::: {#factors-that-influence-performance .section}
## [7.2.1. ]{.section-number}Factors that influence performance[](#factors-that-influence-performance "Link to this heading"){.headerlink}
...
7.  Increase pair style cutoff by 2 [\\(\\AA\\)]{.math .notranslate .nohighlight}: 181.2 katom-step/s
...
:::: {#examples-comparing-parallel-performance .section}
## [7.2.3. ]{.section-number}Examples comparing parallel performance[](#examples-comparing-parallel-performance "Link to this heading"){.headerlink}

The parallel performance usually goes on top of the serial performance. Using twice as many processors should increase the performance metric by up to a factor of two. With the number of processors *N* and the serial performance [\\(p_1\\)]{.math .notranslate .nohighlight} and the performance for *N* processors [\\(p_N\\)]{.math .notranslate .nohighlight} we can define a *parallel efficiency* in percent as follows:

::: {.math .notranslate .nohighlight}
\\\[P\_{eff} = \\frac{p_N}{p_1 \\cdot N} \\cdot 100\\%\\\]
:::
...
- 1 MPI task: 273.6 katom-step/s, [\\(P\_{eff} = 100\\%\\)]{.math .notranslate .nohighlight}

- 2 MPI tasks: 530.6 katom-step/s, [\\(P\_{eff} = 97\\%\\)]{.math .notranslate .nohighlight}

- 4 MPI tasks: 1.021 Matom-step/s, [\\(P\_{eff} = 93\\%\\)]{.math .notranslate .nohighlight}

- 8 MPI tasks: 1.837 Matom-step/s, [\\(P\_{eff} = 84\\%\\)]{.math .notranslate .nohighlight}

- 16 MPI tasks: 3.574 Matom-step/s, [\\(P\_{eff} = 82\\%\\)]{.math .notranslate .nohighlight}

- 32 MPI tasks: 6.479 Matom-step/s, [\\(P\_{eff} = 74\\%\\)]{.math .notranslate .nohighlight}

- 64 MPI tasks: 9.032 Matom-step/s, [\\(P\_{eff} = 52\\%\\)]{.math .notranslate .nohighlight}

- 128 MPI tasks: 12.03 Matom-step/s, [\\(P\_{eff} = 34\\%\\)]{.math .notranslate .nohighlight}
...
- 1 MPI task: 270.8 katom-step/s, [\\(P\_{eff} = 100\\%\\)]{.math .notranslate .nohighlight}

- 2 MPI tasks: 529.3 katom-step/s, [\\(P\_{eff} = 98\\%\\)]{.math .notranslate .nohighlight}

- 4 MPI tasks: 989.8 katom-step/s, [\\(P\_{eff} = 91\\%\\)]{.math .notranslate .nohighlight}

- 8 MPI tasks: 1.832 Matom-step/s, [\\(P\_{eff} = 85\\%\\)]{.math .notranslate .nohighlight}

- 16 MPI tasks: 3.463 Matom-step/s, [\\(P\_{eff} = 80\\%\\)]{.math .notranslate .nohighlight}

- 32 MPI tasks: 5.970 Matom-step/s, [\\(P\_{eff} = 69\\%\\)]{.math .notranslate .nohighlight}

- 64 MPI tasks: 7.477 Matom-step/s, [\\(P\_{eff} = 42\\%\\)]{.math .notranslate .nohighlight}

- 128 MPI tasks: 8.069 Matom-step/s, [\\(P\_{eff} = 23\\%\\)]{.math .notranslate .nohighlight}
::::

::: {#measuring-performance-of-your-input-deck .section}
## [7.2.4. ]{.section-number}Measuring performance of your input deck[](#measuring-performance-of-your-input-deck "Link to this heading"){.headerlink}
...
For the set of runs, look at the timing data printed to the screen and log file at the end of each LAMMPS run. The [[screen and logfile output]{.doc}]Run_output.md){.reference .internal} page gives an overview.
