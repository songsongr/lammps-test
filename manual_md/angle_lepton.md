...
::::::::::::::::::::::: {#angle-style-lepton-command .section}
[]{#index-1}[]{#index-0}

# angle_style lepton command[](#angle-style-lepton-command "Link to this heading"){.headerlink}
...
The Lepton expression must be either enclosed in quotes or must not contain any whitespace so that LAMMPS recognizes it as a single keyword. More on valid Lepton expressions below. The [\\(\\theta_0\\)]{.math .notranslate .nohighlight} coefficient is the "equilibrium angle". It is entered in degrees, but internally converted to radians. Thus the expression must assume "theta" is in radians. The potential energy function in the Lepton expression is shifted in such a way, that the potential energy is 0 for a angle [\\(\\theta_i == \\theta_0\\)]{.math .notranslate .nohighlight}.
::::::
...
This angle style is part of the LEPTON package and only enabled if LAMMPS was built with this package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} page for more info.
:::

::: {#related-commands .section}
## Related commands[](#related-commands "Link to this heading"){.headerlink}

[[angle_coeff]{.doc}]angle_coeff.md){.reference .internal}, [[angle_style table]{.doc}]angle_table.md){.reference .internal}, [[bond_style lepton]{.doc}]bond_lepton.md){.reference .internal},:doc:dihedral_style lepton \<dihedral_lepton\>
:::
