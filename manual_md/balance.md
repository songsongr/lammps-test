...
# balance command[](#balance-command "Link to this heading"){.headerlink}
...
Balancing is performed even if the imbalance factor does not exceed the *thresh* parameter if a "grid" style is specified when the current partitioning is "tiled". The meaning of "grid" vs "tiled" is explained below. This is to allow forcing of the partitioning to "grid" so that the [[comm_style brick]{.doc}]comm_style.md){.reference .internal} command can then be used to replace a current [[comm_style tiled]{.doc}]comm_style.md){.reference .internal} setting.
:::
...
[![balance1](_images/balance_uniform.jpg){style="width: 32%;"}](_images/balance_uniform.jpg){.reference .internal} [![balance2](_images/balance_nonuniform.jpg){style="width: 32%;"}](_images/balance_nonuniform.jpg){.reference .internal} [![balance3](_images/balance_rcb.jpg){style="width: 32%;"}](_images/balance_rcb.jpg){.reference .internal}
...
The "grid" methods can be used with either of the [[comm_style]{.doc}]comm_style.md){.reference .internal} command options, *brick* or *tiled*. The "tiling" methods can only be used with [[comm_style tiled]{.doc}]comm_style.md){.reference .internal}. Note that it can be useful to use a "grid" method with [[comm_style tiled]{.doc}]comm_style.md){.reference .internal} to return the domain partitioning to a logical 3d grid of processors so that "comm_style brick" can afterwords be specified for subsequent [[run]{.doc}]run.md){.reference .internal} commands.
...
The *shift* style invokes a "grid" method for balancing, as described above. It changes the positions of cutting planes between processors in an iterative fashion, seeking to reduce the imbalance factor, similar to how the [[fix balance shift]{.doc}]fix_balance.md){.reference .internal} command operates.
...
This weight style is useful for systems where there are different cutoffs used for different pairs of interactions, or the density fluctuates, or a large number of particles are in the vicinity of a wall, or a combination of these effects. If a simulation uses multiple neighbor lists, this weight style will use the first suitable neighbor list it finds. It will not request or compute a new list. A warning will be issued if there is no suitable neighbor list available or if it is not current, e.g. if the balance command is used before a [[run]{.doc}]run.md){.reference .internal} or [[minimize]{.doc}]minimize.md){.reference .internal} command is used, in which case the neighbor list may not yet have been built. In this case no weights are computed. Inserting a [[run 0 post no]{.doc}]run.md){.reference .internal} command before issuing the *balance* command, may be a workaround for this case, as it will induce the neighbor list to be built.

The *time* weight style uses [[timer data]{.doc}]timer.md){.reference .internal} to estimate weights. It assigns the same weight to each particle owned by a processor based on the total computational time spent by that processor. See details below on what time window is used. It uses the same timing information as is used for the [[MPI task timing breakdown]{.doc}]Run_output.md){.reference .internal}, namely, for sections *Pair*, *Bond*, *Kspace*, and *Neigh*. The time spent in those portions of the timestep are measured for each MPI rank, summed, then divided by the number of particles owned by that processor. I.e. the weight is an effective CPU time/particle averaged over the particles on that processor.
...
The *var* weight style assigns per-particle weights by evaluating an [[atom-style variable]{.doc}]variable.md){.reference .internal} specified by *name*. This is provided as a more flexible alternative to the *group* weight style, allowing definition of a more complex heuristics based on information (global and per atom) available inside of LAMMPS. For example, atom-style variables can reference the position of a particle, its velocity, the volume of its Voronoi cell, etc.

The *store* weight style does not compute a weight factor. Instead it stores the current accumulated weights in a custom per-atom vector specified by *name*. This must be a vector defined as *d_name* via the [[fix property/atom]{.doc}]fix_property_atom.md){.reference .internal} command. This means the values in the vector can be read as part of a data file with the [[read_data]{.doc}]read_data.md){.reference .internal} command or specified with the [[set]{.doc}]set.md){.reference .internal} command. These weights can also be output in a [[dump]{.doc}]dump.md){.reference .internal} file, so this is a way to examine, debug, or visualize the per-particle weights used during the load-balancing operation.
...
Balancing through recursive bisectioning (*rcb* style) requires [[comm_style tiled]{.doc}]comm_style.md){.reference .internal}
:::

::: {#related-commands .section}
## Related commands[](#related-commands "Link to this heading"){.headerlink}

[[group]{.doc}]group.md){.reference .internal}, [[processors]{.doc}]processors.md){.reference .internal}, [[fix balance]{.doc}]fix_balance.md){.reference .internal}, [[comm_style]{.doc}]comm_style.md){.reference .internal}
:::
