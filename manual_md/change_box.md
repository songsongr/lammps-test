...
::::::::::::::::::::::::::::::: {#change-box-command .section}
[]{#index-0}

# change_box command[](#change-box-command "Link to this heading"){.headerlink}
...
The simulation box size/shape can be changed by arbitrarily large amounts by this command. This is not a problem, except that the mapping of processors to the simulation box is not changed from its initial 3d configuration; see the [[processors]{.doc}]processors.md){.reference .internal} command. Thus, if the box size/shape changes dramatically, the mapping of processors to the simulation box may not end up as optimal as the initial mapping attempted to be. You may wish to re-balance the atoms by using the [[balance]{.doc}]balance.md){.reference .internal} command if that is the case.
:::
...
Because the keywords used in this command are applied one at a time to the simulation box and the atoms in it, care must be taken with triclinic cells to avoid exceeding the limits on skew after each transformation in the sequence. If skew is exceeded before the final transformation this can be avoided by changing the order of the sequence, or breaking the transformation into two or more smaller transformations. For more information on the allowed limits for box skew see the discussion on triclinic boxes on [[Howto triclinic]{.doc}]Howto_triclinic.md){.reference .internal} doc page.
:::
...
The *boundary* keyword takes arguments that have exactly the same meaning as they do for the [[boundary]{.doc}]boundary.md){.reference .internal} command. In each dimension, a single letter assigns the same style to both the lower and upper face of the box. Two letters assigns the first style to the lower face and the second style to the upper face.

The style *p* means the box is periodic; the other styles mean non-periodic. For style *f*, the position of the face is fixed. For style *s*, the position of the face is set so as to encompass the atoms in that dimension (shrink-wrapping), no matter how far they move. For style *m*, shrink-wrapping occurs, but is bounded by the current box edge in that dimension, so that the box will become no smaller. See the [[boundary]{.doc}]boundary.md){.reference .internal} command for more explanation of these style options.

Note that the "boundary" command itself can only be used before the simulation box is defined via a [[read_data]{.doc}]read_data.md){.reference .internal} or [[create_box]{.doc}]create_box.md){.reference .internal} or [[read_restart]{.doc}]read_restart.md){.reference .internal} command. This command allows the boundary conditions to be changed later in your input script. Also note that the [[read_restart]{.doc}]read_restart.md){.reference .internal} will change boundary conditions to match what is stored in the restart file. So if you wish to change them, you should use the change_box command after the read_restart command.
...
The simulation box is defined as either orthogonal or triclinic when it is created via the [[create_box]{.doc}]create_box.md){.reference .internal}, [[read_data]{.doc}]read_data.md){.reference .internal}, or [[read_restart]{.doc}]read_restart.md){.reference .internal} commands.

These keywords allow you to toggle the existing simulation box from orthogonal to triclinic and vice versa. For example, an initial equilibration simulation can be run in an orthogonal box, the box can be toggled to triclinic, and then a [[non-equilibrium MD (NEMD) simulation]{.doc}]Howto_nemd.md){.reference .internal} can be run with deformation via the [[fix deform]{.doc}]fix_deform.md){.reference .internal} command.
...
Note that this operation is performed without regard to periodic boundaries. Also, any shrink-wrapping of non-periodic boundaries (see the [[boundary]{.doc}]boundary.md){.reference .internal} command) occurs after all keywords, including this one, have been processed.
...
If you use the *ortho* or *triclinic* keywords, then at the point in the input script when this command is issued, no [[dumps]{.doc}]dump.md){.reference .internal} can be active, nor can a [[fix deform]{.doc}]fix_deform.md){.reference .internal} be active. This is because these commands test whether the simulation box is orthogonal when they are first issued. Note that these commands can be used in your script before a change_box command is issued, so long as an [[undump]{.doc}]undump.md){.reference .internal} or [[unfix]{.doc}]unfix.md){.reference .internal} command is also used to turn them off.
:::

::: {#related-commands .section}
## Related commands[](#related-commands "Link to this heading"){.headerlink}

[[fix deform]{.doc}]fix_deform.md){.reference .internal}, [[boundary]{.doc}]boundary.md){.reference .internal}
:::
