...
:::::::::::::::: {#compute-body-local-command .section}
[]{#index-0}

# compute body/local command[](#compute-body-local-command "Link to this heading"){.headerlink}
...
The local data stored by this command is generated by looping over all the atoms. An atom will only be included if it is in the group. If the atom is a body particle, then its [\\(N\\)]{.math .notranslate .nohighlight} sub-particles will be looped over, and it will contribute [\\(N\\)]{.math .notranslate .nohighlight} data to the count of data. If it is not a body particle, it will contribute 1 datum.
...
For a body particle, the *integer* keywords refer to fields calculated by the body style for each sub-particle. The body style, as specified by the [[atom_style body]{.doc}]atom_style.md){.reference .internal}, determines how many fields exist and what they are. See the [[Howto_body]{.doc}]Howto_body.md){.reference .internal} doc page for details of the different styles.

Here is an example of how to output body information using the [[dump local]{.doc}]dump.md){.reference .internal} command with this compute. If fields 1, 2, and 3 for the body sub-particles are (*x*, *y*, *z*) coordinates, then the dump file will be formatted similar to the output of a [[dump atom or custom]{.doc}]dump.md){.reference .internal} command.
...
The [[units]{.doc}]units.md){.reference .internal} for output values depend on the body style.
:::
