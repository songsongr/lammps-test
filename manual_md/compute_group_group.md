...
::::::::::::::::: {#compute-group-group-command .section}
[]{#index-0}

# compute group/group command[](#compute-group-group-command "Link to this heading"){.headerlink}
...
The energies computed by the *pair* keyword do not include tail corrections, even if they are enabled via the [[pair_modify]{.doc}]pair_modify.md){.reference .internal} command.
:::
...
If the *kspace* keyword is set to *yes*, which is not the default, and if a [[kspace_style]{.doc}]kspace_style.md){.reference .internal} is defined, then the interaction energy will include a Kspace component which is the long-range Coulombic energy between all the atoms in the first group and all the atoms in the second group. Likewise, the interaction force calculated by this compute will include the force on the compute group atoms due to long-range Coulombic interactions with atoms in the specified group2.

Normally the long-range Coulombic energy converges only when the net charge of the unit cell is zero. However, one can assume the net charge of the system is neutralized by a uniform background plasma, and a correction to the system energy can be applied to reduce artifacts. For more information see [[(Bogusz)]{.std .std-ref}](#bogusz){.reference .internal}. If the *boundary* keyword is set to *yes*, which is the default, and *kspace* contributions are included, then this energy correction term will be added to the total group-group energy. This correction term does not affect the force calculation and will be zero if one or both of the groups are charge neutral. This energy correction term is the same as that included in the regular Ewald and PPPM routines.
...
Both the scalar and vector values calculated by this compute are "extensive". The scalar value will be in energy [[units]{.doc}]units.md){.reference .internal}. The vector values will be in force [[units]{.doc}]units.md){.reference .internal}.
:::

::: {#restrictions .section}
## Restrictions[](#restrictions "Link to this heading"){.headerlink}

Not all pair styles can be evaluated in a pairwise mode as required by this compute. For example, three-body and other many-body potentials, such as [[Tersoff]{.doc}]pair_tersoff.md){.reference .internal} and [[Stillinger-Weber]{.doc}]pair_sw.md){.reference .internal} cannot be used. [[EAM]{.doc}]pair_eam.md){.reference .internal} potentials will re-use previously computed embedding term contributions, so the computed pairwise forces and energies are based on the whole system and not valid if particles have been moved since.

Not all [[Kspace styles]{.doc}]kspace_style.md){.reference .internal} support the calculation of group/group interactions. The regular *ewald* and *pppm* styles do.
:::
