...
:::::::::::::::::::: {#compute-msd-chunk-command .section}
[]{#index-0}

# compute msd/chunk command[](#compute-msd-chunk-command "Link to this heading"){.headerlink}
...
Four quantities are calculated by this compute for each chunk. The first 3 quantities are the squared *dx*, *dy*, and *dz* displacements of the center-of-mass. The fourth component is the total squared displacement (i.e., [\\(dx\^2 + dy\^2 + dz\^2\\)]{.math .notranslate .nohighlight}) of the center-of-mass. These calculations include all effects due to atoms passing through periodic boundaries.
...
The simplest way to output the results of the compute msd/chunk calculation to a file is to use the [[fix ave/time]{.doc}]fix_ave_time.md){.reference .internal} command, for example:
...
This compute calculates a global array where the number of rows = the number of chunks *Nchunk* as calculated by the specified [[compute chunk/atom]{.doc}]compute_chunk_atom.md){.reference .internal} command. The number of columns = 4 for *dx*, *dy*, *dz*, and the total displacement. These values can be accessed by any command that uses global array values from a compute as input. See the [[Howto output]{.doc}]Howto_output.md){.reference .internal} page for an overview of LAMMPS output options.

The array values are "intensive". The array values will be in distance[\\(\^2\\)]{.math .notranslate .nohighlight} [[units]{.doc}]units.md){.reference .internal}.
:::
