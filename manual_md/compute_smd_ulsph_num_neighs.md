...
:::::::::::::: {#compute-smd-ulsph-num-neighs-command .section}
[]{#index-0}

# compute smd/ulsph/num/neighs command[](#compute-smd-ulsph-num-neighs-command "Link to this heading"){.headerlink}
...
This compute returns a per-particle vector, which can be accessed by any command that uses per-particle values from a compute as input. See the [[Howto output]{.doc}]Howto_output.md){.reference .internal} page for an overview of LAMMPS output options.
...
[[compute smd/tlsph/num/neighs]{.doc}]compute_smd_tlsph_num_neighs.md){.reference .internal}
:::
