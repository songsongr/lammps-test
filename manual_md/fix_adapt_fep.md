...
:::::::::::::::::::: {#fix-adapt-fep-command .section}
[]{#index-0}

# fix adapt/fep command[](#fix-adapt-fep-command "Link to this heading"){.headerlink}
...
This is an enhanced version of the [[fix adapt]{.doc}]fix_adapt.md){.reference .internal} command with two differences:

- It is possible to modify the charges of chosen atom types only, instead of scaling all the charges in the system.

- There is a new option *after* for better compatibility with [[fix ave/time]{.doc}]fix_ave_time.md){.reference .internal}.

This version is suited for free energy calculations using [[compute ti]{.doc}]compute_ti.md){.reference .internal} or [[compute fep]{.doc}]compute_fep.md){.reference .internal}.
...
The *pstyle* argument is the name of the pair style. If [[pair_style hybrid or hybrid/overlay]{.doc}]pair_hybrid.md){.reference .internal} is used, *pstyle* should be a sub-style name. For example, *pstyle* could be specified as "soft" or "lubricate". The *pparam* argument is the name of the parameter to change. This is the current list of pair styles and parameters that can be varied by this fix. See the doc pages for individual pair styles and their energy formulas for the meaning of these parameters:
  ...
  [[harmonic/cut]{.doc}]pair_harmonic_cut.md){.reference .internal}                                        k                         type pairs
...
It is easy to add new potentials and their parameters to this list. All it typically takes is adding an extract() method to the pair\_\*.cpp file associated with the potential.
:::

Note that for many of the potentials, the parameter that can be varied is effectively a prefactor on the entire energy expression for the potential (e.g., the lj/cut epsilon). The parameters listed as "scale" are exactly that, since the energy expression for the [[coul/cut]{.doc}]pair_coul.md){.reference .internal} potential (for example) has no labeled prefactor in its formula. To apply an effective prefactor to some potentials, multiple parameters need to be altered. For example, the [[Buckingham potential]{.doc}]pair_buck.md){.reference .internal} needs both the A and C terms altered together. To scale the Buckingham potential, you should thus list the pair style twice, once for A and once for C.
...
The [\\(I\\)]{.math .notranslate .nohighlight} argument indicates which atom types are affected. A wild-card asterisk can be used in place of or in conjunction with the [\\(I\\)]{.math .notranslate .nohighlight} argument to set the coefficients for multiple atom types.
...
If the atom parameter is *diameter* and per-atom density and per-atom mass are defined for particles (e.g., [[atom_style granular]{.doc}]atom_style.md){.reference .internal}), then the mass of each particle is also changed when the diameter changes (density is assumed to stay constant).
...
The keyword "scale yes" is not supported for scaling per-atom parameters diameter and change. You can use [[fix adapt]{.doc}]fix_adapt.md){.reference .internal} for those.
:::

::: {#related-commands .section}
## Related commands[](#related-commands "Link to this heading"){.headerlink}

[[compute fep]{.doc}]compute_fep.md){.reference .internal}, [[fix adapt]{.doc}]fix_adapt.md){.reference .internal}, [[compute ti]{.doc}]compute_ti.md){.reference .internal}, [[pair_style \*/soft]{.doc}]pair_fep_soft.md){.reference .internal}
:::
