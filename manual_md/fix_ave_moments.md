...
:::::::::::::::::::: {#fix-ave-moments-command .section}
[]{#index-0}

# fix ave/moments command[](#fix-ave-moments-command "Link to this heading"){.headerlink}
...
Many [[computes]{.doc}]compute.md){.reference .internal} and [[fixes]{.doc}]fix.md){.reference .internal} produce global quantities. See their doc pages for details. [[Variables]{.doc}]variable.md){.reference .internal} of style *equal* and *vector* are the only ones that can be used with this fix. Variables of style *atom* cannot be used, since they produce per-atom values.

The input values must all be scalars or vectors with a bracketed term appended, indicating the [\\(I\^\\text{th}\\)]{.math .notranslate .nohighlight} value of the vector is used.
...
Using a wildcard is the same as if the individual elements of the vector or cells of the array had been listed one by one. For examples, see the description of this capability in [[fix ave/time]{.doc}]fix_ave_time.md){.reference .internal}.

------------------------------------------------------------------------

The [\\(N\_\\text{every}\\)]{.math .notranslate .nohighlight}, [\\(N\_\\text{repeat}\\)]{.math .notranslate .nohighlight}, and [\\(N\_\\text{freq}\\)]{.math .notranslate .nohighlight} arguments specify on what time steps the input values will be used in order to contribute to the average. The final statistics are generated on time steps that are a multiple of [\\(N\_\\text{freq}\\)]{.math .notranslate .nohighlight}. The average is over a window of up to [\\(N\_\\text{repeat}\\)]{.math .notranslate .nohighlight} quantities, computed in the preceding portion of the simulation once every [\\(N\_\\text{every}\\)]{.math .notranslate .nohighlight} time steps.
...
For example, if [\\(N\_\\text{freq}=100\\)]{.math .notranslate .nohighlight} and [\\(N\_\\text{repeat}=5\\)]{.math .notranslate .nohighlight} (and [\\(N\_\\text{every}=1\\)]{.math .notranslate .nohighlight}), then on step 100 values from time steps 96, 97, 98, 99, and 100 will be used. The fix does not compute its inputs on steps that are not required. If [\\(N\_\\text{freq}=5\\)]{.math .notranslate .nohighlight}, [\\(N\_\\text{repeat}=8\\)]{.math .notranslate .nohighlight} and [\\(N\_\\text{every}=1\\)]{.math .notranslate .nohighlight}, then values will first be calculated on step 5 from steps 1-5, on step 10 from 3-10, on step 15 from 8-15 and so on, forming a rolling average over timesteps that span a time window larger than Nfreq.
...
If a value begins with "v\_", a variable name must follow which has been previously defined in the input script. Only equal-style or vector-style variables can be used, which both produce global values. Vector-style variables require a bracketed term to specify the Ith element of the vector calculated by the variable.
...
The moments are output in the order requested in the arguments following the last input. Any number and order of moments can be specified, although it does not make much sense to specify the same moment multiple times. All moments are computed using a correction of the sample estimators used to obtain unbiased cumulants [\\(k\_{1..4}\\)]{.math .notranslate .nohighlight} (see [[(Cramer)]{.std .std-ref}](#cramer1){.reference .internal}). The correction for variance is the standard Bessel correction. For other moments, see [[(Joanes)]{.std .std-ref}](#joanes1){.reference .internal}.

For *mean*, the arithmetic mean [\\(\\bar{x} = \\frac{1}{n} \\sum\_{i=1}\^{n} x_i\\)]{.math .notranslate .nohighlight} is calculated.

For *variance*, the Bessel-corrected sample variance [\\(var = k_2 = \\frac{1}{n - 1} \\sum\_{i=1}\^{n} (x_i - \\bar{x})\^2\\)]{.math .notranslate .nohighlight} is calculated.

For *stddev*, the Bessel-corrected sample standard deviation [\\(stddev = \\sqrt{k_2}\\)]{.math .notranslate .nohighlight} is calculated.

For *skew*, the adjusted Fisher--Pearson standardized moment [\\(G_1 = \\frac{k_3}{k_2\^{3/2}} = \\frac{k_3}{stddev\^3}\\)]{.math .notranslate .nohighlight} is calculated.

For *kurtosis*, the adjusted Fisher--Pearson standardized moment [\\(G_2 = \\frac{k_4}{k_2\^2}\\)]{.math .notranslate .nohighlight} is calculated.
...
To illustrate the utility of keeping output history, consider using this fix in conjunction with [[fix halt]{.doc}]fix_halt.md){.reference .internal} to stop a run automatically if a quantity is converged to within some desired tolerance:
...
This fix produces a global vector and global array which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The values can be accessed on any time step, but may not be current.
...
[[fix ave/time]{.doc}]fix_ave_time.md){.reference .internal},
:::
