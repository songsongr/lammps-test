...
::::::::::::::::::::: {#fix-balance-command .section}
[]{#index-0}

# fix balance command[](#fix-balance-command "Link to this heading"){.headerlink}
...
The weighting options listed above are documented with the [[balance]{.doc}]balance.md){.reference .internal} command in [[this section of the balance command]{.std .std-ref}]balance.md#weighted-balance){.reference .internal} doc page. That section describes the various weighting options and gives a few examples of how they can be used. The weighting options are the same for both the fix balance and [[balance]{.doc}]balance.md){.reference .internal} commands.
:::

Note that the [[processors]{.doc}]processors.md){.reference .internal} command allows some control over how the box volume is split across processors. Specifically, for a [\\(P_x \\times P_y \\times P_z\\)]{.math .notranslate .nohighlight} grid of processors, it allows choices of [\\(P_x\\)]{.math .notranslate .nohighlight}, [\\(P_y\\)]{.math .notranslate .nohighlight}, and [\\(P_z\\)]{.math .notranslate .nohighlight} subject to the constraint that [\\(P_x P_y P_z = P\\)]{.math .notranslate .nohighlight}, the total number of processors. This is sufficient to achieve good load-balance for some problems on some processor counts. However, all the processor subdomains will still have the same shape and the same volume.
...
The "grid" methods can be used with either of the [[comm_style]{.doc}]comm_style.md){.reference .internal} command options, *brick* or *tiled*. The "tiling" methods can only be used with [[comm_style tiled]{.doc}]comm_style.md){.reference .internal}.
...
The [\\(N\_\\text{freq}\\)]{.math .notranslate .nohighlight} setting determines how often a re-balance is performed. If [\\(N\_\\text{freq} \> 0\\)]{.math .notranslate .nohighlight}, then re-balancing will occur every [\\(N\_\\text{freq}\\)]{.math .notranslate .nohighlight} steps. Each time a re-balance occurs, a reneighboring is triggered, so [\\(N\_\\text{freq}\\)]{.math .notranslate .nohighlight} should not be too small. If [\\(N\_\\text{freq} = 0\\)]{.math .notranslate .nohighlight}, then re-balancing will be done every time reneighboring normally occurs, as determined by the the [[neighbor]{.doc}]neighbor.md){.reference .internal} and [[neigh_modify]{.doc}]neigh_modify.md){.reference .internal} command settings.
...
Balancing proceeds by adjusting the cutting planes in each of the dimensions listed in *dimstr*, one dimension at a time. For a single dimension, the balancing operation (described below) is iterated on up to [\\(N\_\\text{iter}\\)]{.math .notranslate .nohighlight} times. After each dimension finishes, the imbalance factor is re-computed, and the balancing operation halts if the *stopthresh* criterion is met.
...
The density-dependent part of this algorithm is often an advantage when you re-balance a system that is already nearly balanced. It typically converges more quickly than the geometric bisectioning algorithm used by the [[balance]{.doc}]balance.md){.reference .internal} command. However, if can be a disadvantage if you attempt to re-balance a system that is far from balanced, and converge more slowly. In this case you probably want to use the [[balance]{.doc}]balance.md){.reference .internal} command before starting a run, so that you begin the run with a balanced system.
...
At each re-balance operation, the bisectioning for each cutting plane (line in 2d) typically starts with low and high bounds separated by the extent of a processor's subdomain in one dimension. The size of this bracketing region shrinks based on the local density, as described above, which should typically be 1/2 or more every iteration. Thus if [\\(N\_\\text{iter}\\)]{.math .notranslate .nohighlight} is specified as 10, the cutting plane will typically be positioned to better than 1 part in 1000 accuracy (relative to the perfect target position). For [\\(N\_\\text{iter} = 20\\)]{.math .notranslate .nohighlight}, it will be accurate to better than 1 part in a million. Thus there is no need to set [\\(N\_\\text{iter}\\)]{.math .notranslate .nohighlight} to a large value. This is especially true if you are re-balancing often enough that each time you expect only an incremental adjustment in the cutting planes is necessary. LAMMPS will check if the threshold accuracy is reached (in a dimension) is less iterations than [\\(N\_\\text{iter}\\)]{.math .notranslate .nohighlight} and exit early.
:::
...
These quantities can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The scalar and vector values calculated by this fix are "intensive".
...
[[group]{.doc}]group.md){.reference .internal}, [[processors]{.doc}]processors.md){.reference .internal}, [[balance]{.doc}]balance.md){.reference .internal}, [[comm_style]{.doc}]comm_style.md){.reference .internal}
:::
