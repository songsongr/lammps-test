...
:::::::::::::: {#fix-dt-reset-command .section}
[]{#index-1}[]{#index-0}

# fix dt/reset command[](#fix-dt-reset-command "Link to this heading"){.headerlink}
...
This fix overrides the timestep size setting made by the [[timestep]{.doc}]timestep.md){.reference .internal} command. The new timestep size *dt* is computed in the following manner.
...
When the [[run style]{.doc}]run_style.md){.reference .internal} is *respa*, this fix resets the outer loop (largest) timestep, which is the same timestep that the [[timestep]{.doc}]timestep.md){.reference .internal} command sets.

Note that the cumulative simulation time (in time units), which accounts for changes in the timestep size as a simulation proceeds, can be accessed by the [[thermo_style time]{.doc}]thermo_style.md){.reference .internal} keyword.

Also note that the [[dump_modify every/time]{.doc}]dump_modify.md){.reference .internal} option allows dump files to be written at intervals specified by simulation time, rather than by timesteps. Simulation time is in time units; see the [[units]{.doc}]units.md){.reference .internal} doc page for details.
...
This fix computes a global scalar which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The scalar stores the last timestep on which the timestep was reset to a new value.
...
[[timestep]{.doc}]timestep.md){.reference .internal}, [[dump_modify every/time]{.doc}]dump_modify.md){.reference .internal}
:::
