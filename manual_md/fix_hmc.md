...
::::::::::::::::::::::: {#fix-hmc-command .section}
[]{#index-0}

# fix hmc command[](#fix-hmc-command "Link to this heading"){.headerlink}
...
\(3\) The energy change [\\(\\Delta{H}\\)]{.math .notranslate .nohighlight} in the Hamiltonian of the system due to the "move" is calculated by the following equation:
...
::: {.math .notranslate .nohighlight}
\\\[p\_{x,y,z} = \\textbf{N}(0,1) \\sqrt{\\frac{k_B T}{2 m\^2}}\\\]
:::
...
The *rigidID* value should be the ID of a [[fix rigid/small]{.doc}]fix_rigid.md){.reference .internal} or [[fix rigid/nve/small]{.doc}]fix_rigid.md){.reference .internal} command which defines the rigid bodies. Its integrator will be used during the MD timesteps. If there are additional particles in the system, e.g. solvent, they should be time-integrated by a [[fix nve]{.doc}]fix_nve.md){.reference .internal} command as explained above.
...
This fix calculates a global scalar and global vector of length 5, which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The scalar is the fraction (0-1) of attempted MC moves which have been accepted. The vector stores the following quantities:
...
These values are updated once every *N* timesteps. The scalar and cumulative counts are "intensive"; the three energies are "extensive" and are in energy [[units]{.doc}]units.md){.reference .internal}.
...
This fix is part of the MC package and requires the RIGID package to be installed. It is only enabled if LAMMPS was built with both packages. See the [[Build package]{.doc}]Build_package.md){.reference .internal} doc page for more info.
:::

::: {#related-commands .section}
## Related commands[](#related-commands "Link to this heading"){.headerlink}

[[fix nvt]{.doc}]fix_nh.md){.reference .internal}, [[fix gcmc]{.doc}]fix_gcmc.md){.reference .internal}, [[fix tfmc]{.doc}]fix_tfmc.md){.reference .internal}
:::
