...
::::::::::::::::: {#fix-imd-command .section}
[]{#index-0}

# fix imd command[](#fix-imd-command "Link to this heading"){.headerlink}
...
This fix implements the "Interactive MD" (IMD) protocol which allows realtime visualization and manipulation of MD simulations through the IMD protocol, as initially implemented in VMD and NAMD. Specifically it allows LAMMPS to connect an IMD client, for example the [VMD visualization program](https://www.ks.uiuc.edu/Research/vmd/){.reference .external} (currently only supports IMDv2) or the [Python IMDClient](https://github.com/Becksteinlab/imdclient/tree/main/imdclient){.reference .external} (supports both IMDv2 and IMDv3), so that it can monitor the progress of the simulation and interactively apply forces to selected atoms.

If LAMMPS is compiled with the pre-processor flag [[-DLAMMPS_ASYNC_IMD]{.std .std-ref}]Build_extras.md#misc){.reference .internal} then fix imd will use POSIX threads to spawn an IMD communication thread on MPI rank 0 in order to offload data exchange with the IMD client from the main execution thread and potentially lower the inferred latencies for slow communication links. This feature has only been tested under linux.
