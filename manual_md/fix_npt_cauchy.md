...
::::::::::::::::::::::::::::: {#fix-npt-cauchy-command .section}
[]{#index-0}

# fix npt/cauchy command[](#fix-npt-cauchy-command "Link to this heading"){.headerlink}
...
The equations of motion used are those of Shinoda et al in [[(Shinoda)]{.std .std-ref}](#nc-shinoda){.reference .internal}, which combine the hydrostatic equations of Martyna, Tobias and Klein in [[(Martyna)]{.std .std-ref}](#nc-martyna){.reference .internal} with the strain energy proposed by Parrinello and Rahman in [[(Parrinello)]{.std .std-ref}](#nc-parrinello){.reference .internal}. The time integration schemes closely follow the time-reversible measure-preserving Verlet and rRESPA integrators derived by Tuckerman et al in [[(Tuckerman)]{.std .std-ref}](#nc-tuckerman){.reference .internal}.
...
A Nose-Hoover thermostat will not work well for arbitrary values of *Tdamp*. If *Tdamp* is too small, the temperature can fluctuate wildly; if it is too large, the temperature will take a very long time to equilibrate. A good choice for many models is a *Tdamp* of around 100 timesteps. Note that this is NOT the same as 100 time units for most [[units]{.doc}]units.md){.reference .internal} settings.
:::
...
The *mtk* keyword controls whether or not the correction terms due to Martyna, Tuckerman, and Klein are included in the equations of motion [[(Martyna)]{.std .std-ref}](#nc-martyna){.reference .internal}. Specifying *no* reproduces the original Hoover barostat, whose volume probability distribution function differs from the true NPT and NPH ensembles by a factor of 1/V. Hence using *yes* is more correct, but in many cases the difference is negligible.

The keyword *tloop* can be used to improve the accuracy of integration scheme at little extra cost. The initial and final updates of the thermostat variables are broken up into *tloop* sub-steps, each of length *dt*/*tloop*. This corresponds to using a first-order Suzuki-Yoshida scheme [[(Tuckerman)]{.std .std-ref}](#nc-tuckerman){.reference .internal}. The keyword *ploop* does the same thing for the barostat thermostat.
...
This fix can be used with either the *verlet* or *respa* [[integrators]{.doc}]run_style.md){.reference .internal}. When using this fix with *respa*, LAMMPS uses an integrator constructed according to the following factorization of the Liouville propagator (for two rRESPA levels):
...
The cumulative energy change in the system imposed by this fix, due to thermostatting and/or barostatting, is included in the [[thermodynamic output]{.doc}]thermo_style.md){.reference .internal} keywords *ecouple* and *econserve*. See the [[thermo_style]{.doc}]thermo_style.md){.reference .internal} page for details.
...
This fix also computes a global vector of quantities, which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. Rhe vector values are "intensive".
...
This fix can ramp its external temperature and pressure over multiple runs, using the *start* and *stop* keywords of the [[run]{.doc}]run.md){.reference .internal} command. See the [[run]{.doc}]run.md){.reference .internal} command for details of how to do this.
