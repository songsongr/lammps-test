...
::: {#fix-orient-fcc-command .section}
[]{#index-1}[]{#index-0}

# fix orient/fcc command[](#fix-orient-fcc-command "Link to this heading"){.headerlink}
:::

::::::::::::::::::: {#fix-orient-bcc-command .section}
# fix orient/bcc command[](#fix-orient-bcc-command "Link to this heading"){.headerlink}
...
The fix applies an orientation-dependent force to atoms near a planar grain boundary which can be used to induce grain boundary migration (in the direction perpendicular to the grain boundary plane). The motivation and explanation of this force and its application are described in [[(Janssens)]{.std .std-ref}](#janssens){.reference .internal}. The adaptation to bcc crystals is described in [[(Wicaksono1)]{.std .std-ref}](#wicaksono1){.reference .internal}. The computed force is only applied to atoms in the fix group.
...
This fix calculates a global scalar which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The scalar is the potential energy change due to this fix. The scalar value calculated by this fix is "extensive".

This fix also calculates a per-atom array which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The array stores the order parameter Xi and normalized order parameter (0 to 1) for each atom. The per-atom values can be accessed on any timestep.
...
These fixes are part of the ORIENT package. They are only enabled if LAMMPS was built with that package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} page for more info.
...
**(Wicaksono2)** Wicaksono, figshare, [https://doi.org/10.6084/m9.figshare.1488628.v1](https://doi.org/10.6084/m9.figshare.1488628.v1){.reference .external} (2015).
