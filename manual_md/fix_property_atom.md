...
::::::::::::::::::::::::::::::: {#fix-property-atom-command .section}
[]{#index-1}[]{#index-0}

# fix property/atom command[](#fix-property-atom-command "Link to this heading"){.headerlink}
...
The atom style used for a simulation defines a set of per-atom properties, as explained on the [[atom_style]{.doc}]atom_style.md){.reference .internal} and [[read_data]{.doc}]read_data.md){.reference .internal} doc pages. The latter command defines these properties for each atom in the system when a data file is read. This fix augments the set of per-atom properties with new custom ones. This can be useful in several scenarios.

If the atom style does not define molecule IDs, per-atom charge, per-atom mass, internal temperature, or internal heat flow, they can be added using the *mol*, *q*, *rmass*, *temperature*, or *heatflow* keywords. This could be useful to define "molecules" to use as rigid bodies with the [[fix rigid]{.doc}]fix_rigid.md){.reference .internal} command, or to carry around an extra flag with atoms (stored as a molecule ID) that can be used by various commands like [[compute chunk/atom]{.doc}]compute_chunk_atom.md){.reference .internal} to group atoms without having to use the group command (which is limited to a total of 32 groups including *all*). For finite-size particles, an internal temperature and heat flow can be used to model heat conduction as in the [[GRANULAR package]{.doc}]Howto_granular.md){.reference .internal}.

Another application is to use the *rmass* flag in order to have per-atom masses instead of per-type masses. This could be used to study isotope effects with partial isotope substitution. [[See below]{.std .std-ref}](#isotopes){.reference .internal} for an example of simulating a mixture of light and heavy water with the TIP4P water potential.
...
If you use the *mol*, *q* or *rmass* names, you most likely want to set *ghost* yes, since these properties are stored with ghost atoms if you use an [[atom_style]{.doc}]atom_style.md){.reference .internal} that defines them. Many LAMMPS operations that use molecule IDs or charge, such as neighbor lists and pair styles, will expect ghost atoms to have these values. LAMMPS will issue a warning it you define those vectors but do not set *ghost* yes.
:::

::: {.note .admonition}
Limitations on ghost atom properties

The specified properties for ghost atoms are not updated every timestep, but only once every few steps when neighbor lists are re-built. Thus the *ghost* keyword is suitable for static properties, like molecule IDs, but not for dynamic properties that change every step. For the latter, the code you add to LAMMPS to change the properties will also need to communicate their new values to/from ghost atoms, an operation that can be invoked from within a [[pair style]{.doc}]pair_style.md){.reference .internal} or [[fix]{.doc}]fix.md){.reference .internal} or [[compute]{.doc}]compute.md){.reference .internal} that you write.
:::

------------------------------------------------------------------------

This fix is one of a small number that can be defined in an input script before the simulation box is created or atoms are defined. This is so it can be used with the [[read_data]{.doc}]read_data.md){.reference .internal} command as described next.

Per-atom properties that are defined by the [[atom style]{.doc}]atom_style.md){.reference .internal} are initialized when atoms are created, e.g. by the [[read_data]{.doc}]read_data.md){.reference .internal} or [[create_atoms]{.doc}]create_atoms.md){.reference .internal} commands. The per-atom properties defined by this fix are not. So you need to initialize them explicitly. One way to do this is [[read_data]{.doc}]read_data.md){.reference .internal} command, using its *fix* keyword and passing it the fix-ID of this fix.
...
Another way of initializing the new properties is via the [[set]{.doc}]set.md){.reference .internal} command. For example, if you wanted molecules defined for every set of 10 atoms, based on their atom-IDs, these commands could be used:
...
The [[atom-style variable]{.doc}]variable.md){.reference .internal} will create values for atoms with IDs 31,32,33,...40 that are 4.0,4.1,4.2,...,4.9. When the [[set]{.doc}]set.md){.reference .internal} commands assigns them to the molecule ID for each atom, they will be truncated to an integer value, so atoms 31-40 will all be assigned a molecule ID of 4.

Note that [[atomfile-style variables]{.doc}]variable.md){.reference .internal} can also be used in place of atom-style variables, which means in this case that the molecule IDs could be read-in from a separate file and assigned by the [[set]{.doc}]set.md){.reference .internal} command. This allows you to initialize new per-atom properties in a completely general fashion.

------------------------------------------------------------------------

For new atom properties specified as *i_name*, *d_name*, *i2_name*, or *d2_name*, the [[dump custom]{.doc}]dump.md){.reference .internal} and [[compute property/atom]{.doc}]compute_property_atom.md){.reference .internal} commands can access their values. This means that the values can be used accessed by fixes like [[fix ave/atom]{.doc}]fix_ave_atom.md){.reference .internal}, accessed by other computes like [[compute reduce]{.doc}]compute_reduce.md){.reference .internal}, or used in [[atom-style variables]{.doc}]variable.md){.reference .internal}.
...
If you wish to add new [[pair styles]{.doc}]pair_style.md){.reference .internal}, [[fixes]{.doc}]fix.md){.reference .internal}, or [[computes]{.doc}]compute.md){.reference .internal} that use the per-atom properties defined by this fix, see the [[Modify atom]{.doc}]Modify_atom.md){.reference .internal} doc page which has details on how the custom properties of this fix can be accessed from added classes.

------------------------------------------------------------------------

Here is an example of using per-atom masses with TIP4P water to study isotope effects. When setting up simulations with the [[TIP4P pair styles]{.doc}]Howto_tip4p.md){.reference .internal} for water, you have to provide exactly one atom type each to identify the water oxygen and hydrogen atoms. Since the atom mass is normally tied to the atom type, this makes it impossible to study multiple isotopes in the same simulation. With *fix property/atom rmass* however, the per-type masses are replaced by per-atom masses. Asumming you have a working input deck for regular TIP4P water, where water oxygen is atom type 1 and water hydrogen is atom type 2, the following lines of input script convert this to using per-atom masses:
...
When writing out the system data with the [[write_data]{.doc}]write_data.md){.reference .internal} command, there will be a new section named with the fix-ID (i.e. *Isotopes* in this case). Alternatively, you can take an existing data file and just add this *Isotopes* section with one line per atom containing atom-ID and mass. Either way, the extended data file can be read back with:
...
This fix writes the per-atom values it stores to [[binary restart files]{.doc}]restart.md){.reference .internal}, so that the values can be restored when a simulation is restarted. See the [[read_restart]{.doc}]read_restart.md){.reference .internal} command for info on how to re-specify a fix in an input script that reads a restart file, so that the operation of the fix continues in an uninterrupted fashion.
...
[[read_data]{.doc}]read_data.md){.reference .internal}, [[set]{.doc}]set.md){.reference .internal}, [[compute property/atom]{.doc}]compute_property_atom.md){.reference .internal}
:::
