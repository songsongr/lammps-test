...
::::::::::::::::::::::: {#fix-qeq-rel-reaxff-command .section}
[]{#index-0}

# fix qeq/rel/reaxff command[](#fix-qeq-rel-reaxff-command "Link to this heading"){.headerlink}
...
This fix implements the QEqR method [[(Lalli)]{.std .std-ref}](#lalli2){.reference .internal} for charge equilibration, which differs from the QEq charge equilibration method [[(Rappe and Goddard)]{.std .std-ref}](#rappe4){.reference .internal} only in how external electric fields are accounted for. This fix therefore raises a warning when used without [[fix efield]{.doc}]fix_efield.md){.reference .internal} since [[fix qeq/reaxff]{.doc}]fix_qeq_reaxff.md){.reference .internal} should be used when no external electric field is present. Charges are computed with the QEqR method by minimizing the electrostatic energy of the system in the same way as the QEq method but where the absolute electronegativity, [\\(\\chi_i\\)]{.math .notranslate .nohighlight}, of each atom in the QEq method is replaced with an effective electronegativity given by

::: {.math .notranslate .nohighlight}
\\\[\\chi\_{\\mathrm{r}i} = \\chi_i + \\frac{\\sum\_{j=1}\^{N} \\beta(\\phi_i - \\phi_j) S\_{ij}} {\\sum\_{m=1}\^{N}S\_{im}},\\\]
:::

where [\\(N\\)]{.math .notranslate .nohighlight} is the number of atoms in the system, [\\(\\beta\\)]{.math .notranslate .nohighlight} is a scaling factor, [\\(\\phi_i\\)]{.math .notranslate .nohighlight} and [\\(\\phi_j\\)]{.math .notranslate .nohighlight} are the electric potentials at the positions of atoms [\\(i\\)]{.math .notranslate .nohighlight} and [\\(j\\)]{.math .notranslate .nohighlight} due to the external electric field and [\\(S\_{ij}\\)]{.math .notranslate .nohighlight} is the overlap integral between atoms [\\(i\\)]{.math .notranslate .nohighlight} and [\\(j\\)]{.math .notranslate .nohighlight}. This formulation is advantageous over the method used by [[fix qeq/reaxff]{.doc}]fix_qeq_reaxff.md){.reference .internal} to account for an external electric field in that it permits periodic boundaries in the direction of an external electric field and in that it does not worsen long-range charge transfer seen with QEq. See [[Lalli]{.std .std-ref}](#lalli2){.reference .internal} for further details.

This fix is typically used in conjunction with the ReaxFF force field model as implemented in the [[pair_style reaxff]{.doc}]pair_reaxff.md){.reference .internal} command, but it can be used with any potential in LAMMPS, so long as it defines and uses charges on each atom. For more technical details about the charge equilibration performed by *fix qeq/rel/reaxff*, which is the same as in [[fix qeq/reaxff]{.doc}]fix_qeq_reaxff.md){.reference .internal} except for the use of [\\(\\chi\_{\\mathrm{r}i}\\)]{.math .notranslate .nohighlight}, please refer to [[(Aktulga)]{.std .std-ref}](#qeq-aktulga3){.reference .internal}. To be explicit, *fix qeq/rel/reaxff* replaces [\\(\\chi_k\\)]{.math .notranslate .nohighlight} of eq. 3 in [[(Aktulga)]{.std .std-ref}](#qeq-aktulga3){.reference .internal} with [\\(\\chi\_{\\mathrm{r}k}\\)]{.math .notranslate .nohighlight} when an external electric field is applied.
...
The optional *scale* keyword sets the value of [\\(\\beta\\)]{.math .notranslate .nohighlight} in the equation for [\\(\\chi\_{\\mathrm{r}i}\\)]{.math .notranslate .nohighlight}. The default value is 1.0.
...
[[pair_style reaxff]{.doc}]pair_reaxff.md){.reference .internal}, [[fix qeq/reaxff]{.doc}]fix_qeq_reaxff.md){.reference .internal}, [[fix acks2/reaxff]{.doc}]fix_acks2_reaxff.md){.reference .internal}, [[fix qtpie/reaxff]{.doc}]fix_qtpie_reaxff.md){.reference .internal}
:::
