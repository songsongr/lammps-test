...
::: {#fix-setforce-command .section}
[]{#index-2}[]{#index-1}[]{#index-0}

# fix setforce command[](#fix-setforce-command "Link to this heading"){.headerlink}

Accelerator Variants: *setforce/kk*
:::

::::::::::::::: {#fix-setforce-spin-command .section}
# fix setforce/spin command[](#fix-setforce-spin-command "Link to this heading"){.headerlink}
...
Any of the 3 quantities defining the force components can be specified as an equal-style or atom-style [[variable]{.doc}]variable.md){.reference .internal}, namely *fx*, *fy*, *fz*. If the value is a variable, it should be specified as v_name, where name is the variable name. In this case, the variable will be evaluated each timestep, and its value used to determine the force component.

Equal-style variables can specify formulas with various mathematical functions, and include [[thermo_style]{.doc}]thermo_style.md){.reference .internal} command keywords for the simulation box parameters and timestep and elapsed time. Thus it is easy to specify a time-dependent force field.
...
The [[fix_modify]{.doc}]fix_modify.md){.reference .internal} *respa* option is supported by this fix. This allows to set at which level of the [[r-RESPA]{.doc}]run_style.md){.reference .internal} integrator the fix is setting the forces to the desired values; on all other levels, the force is set to 0.0 for the atoms in the fix group, so that setforce values are not counted multiple times. Default is to to override forces at the outermost level.
...
The forces due to this fix are imposed during an energy minimization, invoked by the [[minimize]{.doc}]minimize.md){.reference .internal} command, but you cannot set forces to any value besides zero when performing a minimization. Use the [[fix addforce]{.doc}]fix_addforce.md){.reference .internal} command if you want to apply a non-zero force to atoms during a minimization.
:::

::: {#restrictions .section}
## Restrictions[](#restrictions "Link to this heading"){.headerlink}

Fix *setforce/spin* is part of the SPIN package. It is only enabled if LAMMPS was built with that package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} page for more info.
:::

::: {#related-commands .section}
## Related commands[](#related-commands "Link to this heading"){.headerlink}

[[fix addforce]{.doc}]fix_addforce.md){.reference .internal}, [[fix aveforce]{.doc}]fix_aveforce.md){.reference .internal}
:::
