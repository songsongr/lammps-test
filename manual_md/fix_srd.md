...
:::::::::::::::::: {#fix-srd-command .section}
[]{#index-0}

# fix srd command[](#fix-srd-command "Link to this heading"){.headerlink}
...
Treat a group of particles as stochastic rotation dynamics (SRD) particles that serve as a background solvent when interacting with big (colloidal) particles in groupbig-ID. The SRD formalism is described in [[(Hecht)]{.std .std-ref}](#hecht){.reference .internal}. The same methodology is also called multi-particle collision dynamics (MPCD) in the literature.

The key idea behind using SRD particles as a cheap coarse-grained solvent is that SRD particles do not interact with each other, but only with the solute particles, which in LAMMPS can be spheroids, ellipsoids, or line segments, or triangles, or rigid bodies containing multiple spheroids or ellipsoids or line segments or triangles. The collision and rotation properties of the model imbue the SRD particles with fluid-like properties, including an effective viscosity. Thus simulations with large solute particles can be run more quickly, to measure solute properties like diffusivity and viscosity in a background fluid. The usual LAMMPS fixes for such simulations, such as [[fix deform]{.doc}]fix_deform.md){.reference .internal}, [[fix viscosity]{.doc}]fix_viscosity.md){.reference .internal}, and [[fix nvt/sllod]{.doc}]fix_nvt_sllod.md){.reference .internal}, can be used in conjunction with the SRD model.

These 3 papers give more details on how the SRD model is implemented in LAMMPS. [[(Petersen)]{.std .std-ref}](#petersen1){.reference .internal} describes pure SRD fluid systems. [[(Bolintineanu1)]{.std .std-ref}](#bolintineanu1){.reference .internal} describes models where pure SRD fluids interact with boundary walls. [[(Bolintineanu2)]{.std .std-ref}](#bolintineanu2){.reference .internal} describes mixture models where large colloidal particles are solvated by an SRD fluid. See the [`examples/srd`{.docutils .literal .notranslate}]{.pre} directory for sample input scripts.
...
SRD particles have a mass, temperature, characteristic timestep [\\(dt\_{SRD}\\)]{.math .notranslate .nohighlight}, and mean free path between collisions ([\\(\\lambda\\)]{.math .notranslate .nohighlight}). The fundamental equation relating these 4 quantities is

::: {.math .notranslate .nohighlight}
\\\[\\lambda = dt\_{SRD} \\sqrt{\\frac{k_B T\_{SRD}}{m}}\\\]
:::

The mass *m* of SRD particles is set by the [[mass]{.doc}]mass.md){.reference .internal} command elsewhere in the input script. The SRD timestep [\\(dt\_{SRD}\\)]{.math .notranslate .nohighlight} is N times the step *dt* defined by the [[timestep]{.doc}]timestep.md){.reference .internal} command. Big particles move in the normal way via a time integration [[fix]{.doc}]fix.md){.reference .internal} with a short timestep dt. SRD particles advect with a large timestep [\\(dt\_{SRD} \\ge dt\\)]{.math .notranslate .nohighlight}.

If the *lamda* keyword is not specified, the SRD temperature [\\(T\_{SRD}\\)]{.math .notranslate .nohighlight} is used in the above formula to compute [\\(\\lambda\\)]{.math .notranslate .nohighlight}. If the *lamda* keyword is specified, then the *Tsrd* setting is ignored and the above equation is used to compute the SRD temperature.
...
[\\(\\lambda\\)]{.math .notranslate .nohighlight} cannot be smaller than 0.6 \* hgrid, else an error is generated (unless the *shift* keyword is used, see below). The velocities of SRD particles are bounded by Vmax, which is set so that an SRD particle will not advect further than [\\(D\_{max} = 4 \\lambda\\)]{.math .notranslate .nohighlight} in [\\(dt\_{SRD}\\)]{.math .notranslate .nohighlight}. This means that roughly speaking, [\\(D\_{max}\\)]{.math .notranslate .nohighlight} should not be larger than a big particle diameter, else SRDs may pass through big particles without colliding. A warning is generated if this is the case.

Collisions between SRD particles and big particles or walls are modeled as a lightweight SRD point particle hitting a heavy big particle of given diameter or a wall at a point on its surface and bouncing off with a new velocity. The collision changes the momentum of the SRD particle. It imparts a force and torque to the big particle. It imparts a force to a wall. Static or moving SRD walls are setup via the [[fix wall/srd]{.doc}]fix_wall_srd.md){.reference .internal} command. For the remainder of this doc page, a collision of an SRD particle with a wall can be viewed as a collision with a big particle of infinite radius and mass.
...
Three other input script commands will largely overcome this effect, speeding up an SRD simulation by a significant amount. These are the [[atom_modify first]{.doc}]atom_modify.md){.reference .internal}, [[neigh_modify include]{.doc}]neigh_modify.md){.reference .internal}, and [[comm_modify group]{.doc}]comm_modify.md){.reference .internal} commands. Each takes a group-ID as an argument, which in this case should be the group-ID of the big solute particles.

Additionally, when a [[pair_style]{.doc}]pair_style.md){.reference .internal} for big/big particle interactions is specified, the [[pair_coeff]{.doc}]pair_coeff.md){.reference .internal} command should be used to turn off big/SRD interactions, e.g. by setting their epsilon or cutoff length to 0.0.
...
This fix tabulates several SRD statistics which are stored in a vector of length 12, which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The vector values calculated by this fix are "intensive", meaning they do not scale with the size of the simulation. Technically, the first 8 do scale with the size of the simulation, but treating them as intensive means they are not scaled when printed as part of thermodynamic output.
...
This command can only be used if LAMMPS was built with the SRD package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} doc page for more info.
:::
...
The option defaults are: *lamda* ([\\(\\lambda\\)]{.math .notranslate .nohighlight}) is inferred from *Tsrd*, collision = noslip, overlap = no, inside = error, exact = yes, radius = 1.0, bounce = 0, search = hgrid, cubic = error 0.01, shift = no, tstat = no, unbiased = no, and rescale = yes.
