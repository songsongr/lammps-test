...
:::::::::::::::: {#fix-tmd-command .section}
[]{#index-0}

# fix tmd command[](#fix-tmd-command "Link to this heading"){.headerlink}
...
The target file1 can be ASCII text or a compressed text file (detected by a [[supported compression format suffix]{.std .std-ref}]Build_settings.md#gzip){.reference .internal}). The format of the target file1 is as follows:
...
For more information about TMD, see [[(Schlitter1)]{.std .std-ref}](#schlitter1){.reference .internal} and [[(Schlitter2)]{.std .std-ref}](#schlitter2){.reference .internal}.
:::::
...
This fix can ramp its rho parameter over multiple runs, using the *start* and *stop* keywords of the [[run]{.doc}]run.md){.reference .internal} command. See the [[run]{.doc}]run.md){.reference .internal} command for details of how to do this.
...
To read compressed target files, you must compile LAMMPS with the [`-DLAMMPS_GZIP`{.docutils .literal .notranslate}]{.pre} option. See the [[Build settings]{.doc}]Build_settings.md){.reference .internal} doc page for details.
:::
