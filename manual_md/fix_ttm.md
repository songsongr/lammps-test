...
::: {#fix-ttm-command .section}
[]{#index-3}[]{#index-2}[]{#index-1}[]{#index-0}

# fix ttm command[](#fix-ttm-command "Link to this heading"){.headerlink}
:::

::: {#fix-ttm-grid-command .section}
# fix ttm/grid command[](#fix-ttm-grid-command "Link to this heading"){.headerlink}
:::

::: {#fix-ttm-mod-command .section}
# fix ttm/mod command[](#fix-ttm-mod-command "Link to this heading"){.headerlink}
:::

:::::::::::::::::::::::::::::: {#fix-ttm-thermal-command .section}
# fix ttm/thermal command[](#fix-ttm-thermal-command "Link to this heading"){.headerlink}
...
Fix *ttm/thermal* allows for electronic properties to be assigned independently to each TTM grid point and supports external heat sources to the electronic subsystem. More details on fix *ttm/thermal* are given below.
...
where [\\(C_e\\)]{.math .notranslate .nohighlight} is the specific heat, [\\(\\rho_e\\)]{.math .notranslate .nohighlight} is the density, [\\(\\kappa_e\\)]{.math .notranslate .nohighlight} is the thermal conductivity, *T* is temperature, the "e" and "a" subscripts represent electronic and atomic subsystems respectively, [\\(g_p\\)]{.math .notranslate .nohighlight} is the coupling constant for the electron-ion interaction, and [\\(g_s\\)]{.math .notranslate .nohighlight} is the electron stopping coupling parameter. [\\(C_e\\)]{.math .notranslate .nohighlight}, [\\(\\rho_e\\)]{.math .notranslate .nohighlight}, and [\\(\\kappa_e\\)]{.math .notranslate .nohighlight} are specified as parameters to the fix *ttm* or *ttm/grid*. The other quantities are derived. The form of the heat diffusion equation used here is almost the same as that in equation 6 of [[(Duffy)]{.std .std-ref}](#duffy){.reference .internal}, with the exception that the electronic density is explicitly represented, rather than being part of the specific heat parameter.

Currently, the TTM fixes assume that none of the user-supplied parameters will vary with temperature. Note that [[(Duffy)]{.std .std-ref}](#duffy){.reference .internal} used a tanh() functional form for the temperature dependence of the electronic specific heat, but ignored temperature dependencies of any of the other parameters. See more discussion below for fix *ttm/mod*.

::: {.admonition .note}
Note

These fixes do not perform time integration of the atoms in the fix group, they only rescale their velocities. Thus a time integration fix such as [[fix nve]{.doc}]fix_nve.md){.reference .internal} should be used in conjunction with these fixes. These fixes should not normally be used on atoms that have their temperature controlled by another thermostatting fix, e.g. [[fix nvt]{.doc}]fix_nh.md){.reference .internal} or [[fix langevin]{.doc}]fix_langevin.md){.reference .internal}.
:::

::: {.admonition .note}
Note

These fixes require use of an orthogonal 3d simulation box with periodic boundary conditions in all dimensions. They also require that the size and shape of the simulation box do not vary dynamically, e.g. due to use of the [[fix npt]{.doc}]fix_nh.md){.reference .internal} command. Likewise, the size/shape of processor subdomains cannot vary due to dynamic load-balancing via use of the [[fix balance]{.doc}]fix_balance.md){.reference .internal} command. It is possible however to load balance before the simulation starts using the [[balance]{.doc}]balance.md){.reference .internal} command, so that each processor has a different size subdomain.
:::

Periodic boundary conditions are also used in the heat equation solve for the electronic subsystem. This varies from the approach of [[(Rutherford)]{.std .std-ref}](#rutherford){.reference .internal} where the atomic subsystem was embedded within a larger continuum representation of the electronic subsystem.
...
The fix ttm/grid command does not support the *outfile* keyword. Instead you can use the [[dump grid]{.doc}]dump.md){.reference .internal} command to output the electronic temperature on the distributed grid to a dump file or the [[restart]{.doc}]restart.md){.reference .internal} command which creates a file specific to this fix which the [[read restart]{.doc}]read_restart.md){.reference .internal} command reads. The file has the same format as the file the *infile* option reads.
:::

For the fix ttm, fix ttm/mod, and fix ttm/thermal commands, the corresponding atomic temperature for atoms in each grid cell can be computed and output by the [[fix ave/chunk]{.doc}]fix_ave_chunk.md){.reference .internal} command using the [[compute chunk/atom]{.doc}]compute_chunk_atom.md){.reference .internal} command to create a 3d array of chunks consistent with the grid used by this fix.

For the fix ttm/grid command the same thing can be done using the [[fix ave/grid]{.doc}]fix_ave_grid.md){.reference .internal} command and its per-grid values can be output via the [[dump grid]{.doc}]dump.md){.reference .internal} command.
...
::: {.math .notranslate .nohighlight}
\\\[C_e \\rho_e \\frac{\\partial T_e}{\\partial t} = \\bigtriangledown (\\kappa_e \\bigtriangledown T_e) - g_p (T_e - T_a) + g_s T_a\' + \\theta (x-x\_{surface})I_0 \\exp(-x/l\_{skin})\\\]
:::

where [\\(\\theta\\)]{.math .notranslate .nohighlight} is the Heaviside step function, [\\(I_0\\)]{.math .notranslate .nohighlight} is the (absorbed) laser pulse intensity for ablation simulations, [\\(l\_{skin}\\)]{.math .notranslate .nohighlight} is the depth of the skin-layer, and all other designations have the same meaning as in the former equation. The duration of the pulse is set by the parameter *tau* in the *init_file*.

Fix *ttm/mod* also allows users to specify the dependencies of [\\(C_e\\)]{.math .notranslate .nohighlight} and [\\(\\kappa_e\\)]{.math .notranslate .nohighlight} on the electronic temperature. The specific heat is expressed as

::: {.math .notranslate .nohighlight}
\\\[C_e = C_0 + (a_0 + a_1 X + a_2 X\^2 + a_3 X\^3 + a_4 X\^4) \\exp (-(AX)\^2)\\\]
:::

where [\\(X = \\frac{T_e}{1000}\\)]{.math .notranslate .nohighlight}, and the thermal conductivity is defined as [\\(\\kappa_e = D_e \\cdot rho_e \\cdot C_e\\)]{.math .notranslate .nohighlight}, where [\\(D_e\\)]{.math .notranslate .nohighlight} is the thermal diffusion coefficient.

Electronic pressure effects are included in the TTM model to account for the blast force acting on ions because of electronic pressure gradient (see [[(Chen)]{.std .std-ref}](#chen){.reference .internal}, [[(Norman)]{.std .std-ref}](#norman){.reference .internal}). The total force acting on an ion is:

::: {.math .notranslate .nohighlight}
\\\[{\\vec F}\_i = - \\partial U / \\partial {\\vec r}\_i + {\\vec F}\_{langevin} - \\nabla P_e/n\_{ion}\\\]
:::

where [\\(F\_{langevin}\\)]{.math .notranslate .nohighlight} is a force from Langevin thermostat simulating electron-phonon coupling, and [\\(\\nabla P_e/n\_{ion}\\)]{.math .notranslate .nohighlight} is the electron blast force.

The electronic pressure is taken to be [\\(P_e = B \\cdot rho_e \\cdot C_e \\cdot T_e\\)]{.math .notranslate .nohighlight}
...
where [\\(\\lambda\\)]{.math .notranslate .nohighlight} is the electron mean free path (see [[(Norman)]{.std .std-ref}](#norman){.reference .internal}, [[(Pisarev)]{.std .std-ref}](#pisarev){.reference .internal})
...
::: {.math .notranslate .nohighlight}
\\\[C\_\\mathrm{vol} \\frac{\\partial T_e}{\\partial t} = \\bigtriangledown (\\kappa\_\\mathrm{eff} \\bigtriangledown T_e) - g_p (T_e - T_a) + \\eta s\\\]
:::

where [\\(s\\)]{.math .notranslate .nohighlight} is the applied heating power density and [\\(\\eta\\)]{.math .notranslate .nohighlight} is the absorption efficiency (0-1) defined for each ttm grid cell in the *properties.in* file. Also note that compared to the original *fix ttm*, it uses use a volumetric specific heat, [\\(C\_\\mathrm{vol}\\)]{.math .notranslate .nohighlight} , which represents the product of [\\(C_e \\rho_e\\)]{.math .notranslate .nohighlight}.

[[(Baer)]{.std .std-ref}](#baer){.reference .internal} defined [\\(\\kappa\_\\mathrm{eff}\\)]{.math .notranslate .nohighlight} as an effective electronic thermal conductivity when two adjacent TTM cells (denoted by the subscripts *a* and *b*) have different conductivities as:

::: {.math .notranslate .nohighlight}
\\\[\\kappa\_\\mathrm{eff} = \\frac{2 \\kappa_a \\kappa_b}{\\kappa_a + \\kappa_b}\\\]
:::

The current fix *ttm/thermal* implementation allows TTM simulations with TTM cells that do not contain electrons (vacuum or insulators). Similar to *ttm/mod*, the absence of electrons is defined as the grid cells with zero electronic temperature.The numerical scheme does not allow energy exchange with such cells.
...
the grid must match the one declared in the fix and all grid points must have all properties set or *ttm/thermal* will exit with an error.
:::::::::::::::::::
...
The fix ttm, fix ttm/mod, and fix ttm/thermal commands write the state of the electronic subsystem and the energy exchange between the subsystems to [[binary restart files]{.doc}]restart.md){.reference .internal}. The fix ttm/grid command does not yet support writing of its distributed grid to a restart file.
...
None of the [[fix_modify]{.doc}]fix_modify.md){.reference .internal} options are relevant to these fixes.

These fixes compute 2 output quantities stored in a vector of length 2, which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The first quantity is the total energy of the electronic subsystem. The second quantity is the energy transferred from the electronic to the atomic subsystem on that timestep. Note that the velocity verlet integrator applies the fix ttm forces to the atomic subsystem as two half-step velocity updates: one on the current timestep and one on the subsequent timestep. Consequently, the change in the atomic subsystem energy is lagged by half a timestep relative to the change in the electronic subsystem energy. As a result of this, users may notice slight fluctuations in the sum of the atomic and electronic subsystem energies reported at the end of the timestep.

The vector values calculated are "extensive".

The fix ttm/grid command also outputs a per-grid vector which stores the electron temperature for each grid cell in temperature [[units]{.doc}]units.md){.reference .internal}. which can be accessed by various [[output commands]{.doc}]Howto_output.md){.reference .internal}. The length of the vector (distributed across all processors) is Nx \* Ny \* Nz. For access by other commands, the name of the single grid produced by fix ttm/grid is "grid". The name of its per-grid data is "data".

No parameter of the fixes can be used with the *start/stop* keywords of the [[run]{.doc}]run.md){.reference .internal} command. The fixes are not invoked during [[energy minimization]{.doc}]minimize.md){.reference .internal}.
:::

::: {#restrictions .section}
## Restrictions[](#restrictions "Link to this heading"){.headerlink}

All these fixes are part of the EXTRA-FIX package. They are only enabled if LAMMPS was built with that package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} page for more info.
...
[[fix langevin]{.doc}]fix_langevin.md){.reference .internal}, [[fix dt/reset]{.doc}]fix_dt_reset.md){.reference .internal}
:::
