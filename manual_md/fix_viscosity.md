...
:::::::::::::::::: {#fix-viscosity-command .section}
[]{#index-0}

# fix viscosity command[](#fix-viscosity-command "Link to this heading"){.headerlink}
...
As described below, the total momentum transferred by these velocity swaps is computed by the fix and can be output. Dividing this quantity by time and the cross-sectional area of the simulation box yields a momentum flux. The ratio of momentum flux to the slope of the shear velocity profile is proportional to the viscosity of the fluid, in appropriate units. See the [[Muller-Plathe paper]{.std .std-ref}](#muller-plathe2){.reference .internal} for details.
...
An alternative method for calculating a viscosity is to run a NEMD simulation, as described on the [[Howto nemd]{.doc}]Howto_nemd.md){.reference .internal} doc page. NEMD simulations deform the simulation box via the [[fix deform]{.doc}]fix_deform.md){.reference .internal} command.
...
[[fix ave/chunk]{.doc}]fix_ave_chunk.md){.reference .internal}, [[fix thermal/conductivity]{.doc}]fix_thermal_conductivity.md){.reference .internal}
:::
