...
::::::::::::::::: {#geturl-command .section}
[]{#index-0}

# geturl command[](#geturl-command "Link to this heading"){.headerlink}
...
Download a file from an URL to the local disk. This is implemented with the [libcurl library](https://curl.se/libcurl/){.reference .external} which supports a large variety of protocols including "http", "https", "ftp", "scp", "sftp", "file". The transfer will only be performed on MPI rank 0.
...
The *verify* keyword determines whether [`libcurl`{.docutils .literal .notranslate}]{.pre} will validate the SSL certificate and hostname for encrypted connections. Turning this off may be required when using a proxy or connecting to a server with a self-signed SSL certificate.
...
::::: {#geturl-proxy .note .admonition}
Using *geturl* with proxies for http or https

The [libcurl library](https://curl.se/libcurl/){.reference .external} supports [routing traffic through proxies](https://everything.curl.dev/usingcurl/proxies/env.html){.reference .external} by setting suitable environment variables (e.g. [`http_proxy`{.docutils .literal .notranslate}]{.pre} or [`https_proxy`{.docutils .literal .notranslate}]{.pre}) as required by some institutional or corporate security protocols. In that case you probably also want to use the *verify* *no* setting.
...
This command is part of the EXTRA-COMMAND package. It is only enabled if LAMMPS was built with that package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} page for more info. It also requires that LAMMPS was built with support for [the libcurl library](https://curl.se/libcurl/){.reference .external}. See the page about [[Compiling LAMMPS with libcurl support]{.std .std-ref}]Build_settings.md#libcurl){.reference .internal} for further info. If support for libcurl is not included, using *geturl* will trigger an error.
:::

::: {#related-commands .section}
## Related commands[](#related-commands "Link to this heading"){.headerlink}

[[shell]{.doc}]shell.md){.reference .internal}
:::
