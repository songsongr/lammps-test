...
# group command[](#group-command "Link to this heading"){.headerlink}
...
Identify a collection of atoms as belonging to a group. The group ID can then be used in other commands such as [[fix]{.doc}]fix.md){.reference .internal}, [[compute]{.doc}]compute.md){.reference .internal}, [[dump]{.doc}]dump.md){.reference .internal}, or [[velocity]{.doc}]velocity.md){.reference .internal} to act on those atoms together.
...
The *delete* style removes the named group and un-assigns all atoms that were assigned to that group. Since there is a restriction (see below) that no more than 32 groups can be defined at any time, the *delete* style allows you to remove groups that are no longer needed, so that more can be specified. You cannot delete a group if it has been used to define a current [[fix]{.doc}]fix.md){.reference .internal} or [[compute]{.doc}]compute.md){.reference .internal} or [[dump]{.doc}]dump.md){.reference .internal}.

The *clear* style un-assigns all atoms that were assigned to that group. This may be dangerous to do during a simulation run (e.g., using the [[run every]{.doc}]run.md){.reference .internal} command if a fix or compute or other operation expects the atoms in the group to remain constant), but LAMMPS does not check for this.

The *empty* style creates an empty group, which is useful for commands like [[fix gcmc]{.doc}]fix_gcmc.md){.reference .internal} or with complex scripts that add atoms to a group.

The *region* style puts all atoms in the region volume into the group. Note that this is a static one-time assignment. The atoms remain assigned (or not assigned) to the group even if they later move out of the region volume.
...
The first format is a list of values (types or IDs). For example, the second command in the examples above puts all atoms of type 3 or 4 into the group named *water*. Each numeric entry in the list can be a colon-separated sequence [`A:B`{.docutils .literal .notranslate}]{.pre} or [`A:B:C`{.docutils .literal .notranslate}]{.pre}, as in two of the examples above. A "sequence" generates a sequence of values (types or IDs), with an optional increment. The first example with [`500:1000`{.docutils .literal .notranslate}]{.pre} has the default increment of 1 and would add all atom IDs from 500 to 1000 (inclusive) to the group sub, along with 10, 25, and 50 since they also appear in the list of values. The second example with [`100:10000:10`{.docutils .literal .notranslate}]{.pre} uses an increment of 10 and would thus add atoms IDs [\\(100, 110, 120, \\dots, 9990, 10000\\)]{.math .notranslate .nohighlight} to the group sub.

The second format is a *logical* followed by one or two values (type or ID). The 7 valid logicals are listed above. All the logicals except [`<>`{.docutils .literal .notranslate}]{.pre} take a single argument. The third example above adds all atoms with IDs from 1 to 150 to the group named *sub*. The logical [`<>`{.docutils .literal .notranslate}]{.pre} means "between" and takes 2 arguments. The fourth example above adds all atoms belonging to molecules with IDs from 50 to 250 (inclusive) to the group named polyA. For the *type* style, type labels are converted into numeric types before being evaluated.

The *variable* style evaluates a variable to determine which atoms to add to the group. It must be an [[atom-style variable]{.doc}]variable.md){.reference .internal} previously defined in the input script. If the variable evaluates to a non-zero value for a particular atom, then that atom is added to the specified group.
...
For example, these lines define a variable "eatom" that calculates the potential energy of each atom and includes it in the group if its potential energy is above the threshold value [\\(-3.0\\)]{.math .notranslate .nohighlight}.
...
[[dump]{.doc}]dump.md){.reference .internal}, [[fix]{.doc}]fix.md){.reference .internal}, [[region]{.doc}]region.md){.reference .internal}, [[velocity]{.doc}]velocity.md){.reference .internal}
:::
