...
::::::::::::::::: {#improper-coeff-command .section}
[]{#index-0}

# improper_coeff command[](#improper-coeff-command "Link to this heading"){.headerlink}
...
Specify the improper force field coefficients for one or more improper types. The number and meaning of the coefficients depends on the improper style. Improper coefficients can also be set in the data file read by the [[read_data]{.doc}]read_data.md){.reference .internal} command or in a restart file.
...
The [[improper_style class2]{.doc}]improper_class2.md){.reference .internal} is an exception to this rule, in that an additional argument is used in the input script to allow specification of the cross-term coefficients. See its doc page for details.

------------------------------------------------------------------------

The list of all improper styles defined in LAMMPS is given on the [[improper_style]{.doc}]improper_style.md){.reference .internal} doc page. They are also listed in more compact form on the [[Commands improper]{.std .std-ref}]Commands_bond.md#improper){.reference .internal} doc page.
