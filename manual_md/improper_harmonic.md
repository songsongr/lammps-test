...
::::::::::::::: {#improper-style-harmonic-command .section}
[]{#index-3}[]{#index-2}[]{#index-1}[]{#index-0}

# improper_style harmonic command[](#improper-style-harmonic-command "Link to this heading"){.headerlink}
...
where [\\(\\chi\\)]{.math .notranslate .nohighlight} is the improper angle, [\\(\\chi_0\\)]{.math .notranslate .nohighlight} is its equilibrium value, and [\\(K\\)]{.math .notranslate .nohighlight} is a prefactor. Note that the usual 1/2 factor is included in [\\(K\\)]{.math .notranslate .nohighlight}.

If the 4 atoms in an improper quadruplet (listed in the data file read by the [[read_data]{.doc}]read_data.md){.reference .internal} command) are ordered I,J,K,L then [\\(\\chi\\)]{.math .notranslate .nohighlight} is the angle between the plane of I,J,K and the plane of J,K,L. Alternatively, you can think of atoms J,K,L as being in a plane, and atom I above the plane, and [\\(\\chi\\)]{.math .notranslate .nohighlight} as a measure of how far out-of-plane I is with respect to the other 3 atoms.
...
For the *harmonic* improper style, the first atom in the quadruplet is the atom of symmetry; all other atoms are considered interchangeable. This convention is relevant for operations that require knowledge of how atoms are ordered, such as automatic assignment of new improper types by [[fix bond/react]{.doc}]fix_bond_react.md){.reference .internal}.
:::
