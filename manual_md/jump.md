...
:::::::::::::::::::::::::::: {#jump-command .section}
[]{#index-0}

# jump command[](#jump-command "Link to this heading"){.headerlink}
...
This command closes the current input script file, opens the file with the specified name, and begins reading LAMMPS commands from that file. Unlike the [[include]{.doc}]include.md){.reference .internal} command, the original file is not returned to, although by using multiple jump commands it is possible to chain from file to file or back to the original file.
...
since the SELF option invokes the C-library rewind() call, which may not be supported for stdin on some systems or by some MPI implementations. This can be worked around by using the [[-in command-line switch]{.doc}]Run_options.md){.reference .internal}, e.g.
...
or by using the [[-var command-line switch]{.doc}]Run_options.md){.reference .internal} to pass the script name as a variable to the input script. In the latter case, a [[variable]{.doc}]variable.md){.reference .internal} called "fname" could be used in place of SELF, e.g.
...
Here is an example of a double loop which uses the if and [[jump]{.doc}](#){.reference .internal} commands to break out of the inner loop when a condition is met, then continues iterating through the outer loop.
...
[[variable]{.doc}]variable.md){.reference .internal}, [[include]{.doc}]include.md){.reference .internal}, [[label]{.doc}]label.md){.reference .internal}, [[next]{.doc}]next.md){.reference .internal}
:::
