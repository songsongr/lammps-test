...
# kim command[](#kim-command "Link to this heading"){.headerlink}
...
- sub-command = [[init]{.std .std-ref}](#kim-init){.reference .internal} or [[interactions]{.std .std-ref}](#interactions){.reference .internal} or [[query]{.std .std-ref}](#query){.reference .internal} or [[param]{.std .std-ref}](#param){.reference .internal} or [[property]{.std .std-ref}](#property){.reference .internal}
...
The *kim command* includes a set of sub-commands that allow LAMMPS users to use interatomic models (IM) (potentials and force fields) and their predictions for various physical properties archived in the [Open Knowledgebase of Interatomic Models (OpenKIM)](https://openkim.org){.reference .external} repository.

Using OpenKIM provides LAMMPS users with immediate access to a large number of verified IMs and their predictions. OpenKIM IMs have multiple benefits including [reliability, reproducibility and convenience](https://openkim.org/doc/overview/kim-features/){.reference .external}.

There are two types of IMs archived in OpenKIM:

1.  The first type is called a *KIM Portable Model* (PM). A KIM PM is an independent computer implementation of an IM written in one of the languages supported by KIM (C, C++, Fortran) that conforms to the KIM Application Programming Interface ([KIM API](https://openkim.org/kim-api/){.reference .external}) Portable Model Interface (PMI) standard. A KIM PM will work seamlessly with any simulation code that supports the KIM API/PMI standard (including LAMMPS; see [complete list of supported codes](https://openkim.org/projects-using-kim/){.reference .external}).
...
With these two IM types, OpenKIM can archive and test almost all IMs that can be used by LAMMPS. (It is easy to contribute new IMs to OpenKIM, see the [upload instructions](https://openkim.org/doc/repository/adding-content/){.reference .external}.)

OpenKIM IMs are uniquely identified by a [KIM ID](https://openkim.org/doc/schema/kim-ids/){.reference .external}. The extended KIM ID consists of a human-readable prefix identifying the type of IM, authors, publication year, and supported species, separated by two underscores from the KIM ID itself, which begins with an IM code (*MO* for a KIM Portable Model, and *SM* for a KIM Simulator Model) followed by a unique 12-digit code and a 3-digit version identifier. By convention SM prefixes begin with *Sim\_* to readily identify them.
...
Each OpenKIM IM has a dedicated "Model Page" on [OpenKIM](https://openkim.org){.reference .external} providing all the information on the IM including a title, description, authorship and citation information, test and verification check results, visualizations of results, a wiki with documentation and user comments, and access to raw files, and other information. The URL for the Model Page is constructed from the [extended KIM ID](https://openkim.org/doc/schema/kim-ids/){.reference .external} of the IM:
...
See the [current list of KIM PMs and SMs archived in OpenKIM](https://openkim.org/browse/models/by-species){.reference .external}. This list is sorted by species and can be filtered to display only IMs for certain species combinations.

See [Obtaining KIM Models](https://openkim.org/doc/usage/obtaining-models){.reference .external} to learn how to install a pre-built binary of the OpenKIM Repository of Models.
...
:::::::::::::::::::::::::::::::::::: {#using-openkim-ims-with-lammps-kim-init-kim-interactions .section}
## Using OpenKIM IMs with LAMMPS (*kim init*, *kim interactions*)[](#using-openkim-ims-with-lammps-kim-init-kim-interactions "Link to this heading"){.headerlink}
...
- user_units = the LAMMPS [[units]{.doc}]units.md){.reference .internal} style assumed in the LAMMPS input script
...
::::: {#id2 .section}
### Examples[](#id2 "Link to this heading"){.headerlink}
...
:::::::::::: {#openkim-im-initialization-kim-init .section}
[]{#kim-init}

### OpenKIM IM Initialization (*kim init*)[](#openkim-im-initialization-kim-init "Link to this heading"){.headerlink}

The *kim* command followed by *init* sub-command must be issued **before** the simulation box is created (normally at the top of the file). This command sets the OpenKIM IM that will be used and may issue additional commands changing LAMMPS default settings that are required for using the selected IM (such as [[units]{.doc}]units.md){.reference .internal} or [[atom_style]{.doc}]atom_style.md){.reference .internal}). If needed, those settings can be overridden, however, typically a script containing a *kim init* command would not include *units* and *atom_style* commands.

The required arguments of *kim init* are the *model* name of the IM to be used in the simulation (for an IM archived in OpenKIM this is its [extended KIM ID](https://openkim.org/doc/schema/kim-ids/){.reference .external}, and the *user_units*, which are the LAMMPS [[units style]{.doc}]units.md){.reference .internal} used in the input script. (Any dimensioned numerical values in the input script and values read in from files are expected to be in the *user_units* system.)

The selected IM can be either a [[KIM PM or a KIM SM]{.std .std-ref}](#im-types){.reference .internal}. For a KIM SM, the *kim init* command verifies that the SM is designed to work with LAMMPS (and not another simulation code). In addition, the LAMMPS version used for defining the SM and the LAMMPS version being currently run are printed to help diagnose any incompatible changes to input script or command syntax between the two LAMMPS versions.

Based on the selected model *kim init* may modify the [[atom_style]{.doc}]atom_style.md){.reference .internal}. Some SMs have requirements for this setting. If this is the case, then *atom_style* will be set to the required style. Otherwise, the value is left unchanged (which in the absence of an *atom_style* command in the input script is the [[default atom_style value]{.doc}]atom_style.md){.reference .internal}).

Regarding units, the *kim init* behaves in different ways depending on whether or not *unit conversion mode* is activated as indicated by the optional *unitarg* argument. If unit conversion mode is **not** active, then *user_units* must either match the required units of the IM or the IM must be able to adjust its units to match. (The latter is only possible with some KIM PMs; SMs can never adjust their units.) If a match is possible, the LAMMPS [[units]{.doc}]units.md){.reference .internal} command is called to set the units to *user_units*. If the match fails, the simulation is terminated with an error. The *kim init* command also sets the default value for the [[skin]{.doc}]neighbor.md){.reference .internal} (extra distance beyond force cutoff) as 2.0 Angstroms and sets the default value for the [[timestep]{.doc}]timestep.md){.reference .internal} size as 1.0 femtosecond.
...
If unit conversion mode *is* active, then *kim init* calls the LAMMPS [[units]{.doc}]units.md){.reference .internal} command to set the units to the IM's required or preferred units. Conversion factors between the IM's units and the *user_units* are defined for all [[physical quantities]{.doc}]units.md){.reference .internal} (mass, distance, etc.). (Note that converting to or from the "lj" unit style is not supported.) These factors are stored as [[internal style variables]{.doc}]variable.md){.reference .internal} with the following standard names:
...
If desired, the input script can be designed to work with these conversion factors so that the script will work without change with any OpenKIM IM. (This approach is used in the [OpenKIM Testing Framework](https://openkim.org/doc/evaluation/kim-tests/){.reference .external}.)
...
Note the multiplication by v\_\_u_distance and v\_\_u_mass to convert from SI units (specified in the *kim init* command) to whatever units the IM uses (metal in this case), and the division by v\_\_u_energy to convert from the IM's energy units to SI units (Joule). This script will work correctly for any IM for Al (KIM PM or SM) selected by the *kim init* command.

Care must be taken to apply unit conversion to dimensional variables read in from a file. For example, if a configuration of atoms is read in from a dump file using the [[read_dump]{.doc}]read_dump.md){.reference .internal} command, the following can be done to convert the box and all atomic positions to the correct units:
...
*kim interactions* must be called each time after the [[change_box]{.doc}]change_box.md){.reference .internal} command to provide the correct settings (it should be called with the same [[typeargs]{.std .std-ref}](#typeargs-options){.reference .internal} as the first call.) The reason is that changing a periodic boundary to a non-periodic one, or in general, using the *change_box* command after the interactions are set via *kim interactions* or *pair_coeff* commands might affect some of the settings. For example, SM models containing Coulombic terms in the interactions require different settings if a periodic boundary changes to a non-periodic one. In other cases, the second call to *kim interactions* does not affect any other settings.
:::
:::::::::::::::::::
::::::::::::::::::::::::::::::::::::

::::::::::::::::::::::::::::: {#using-openkim-web-queries-in-lammps-kim-query .section}
[]{#query}

## Using OpenKIM Web Queries in LAMMPS (*kim query*)[](#using-openkim-web-queries-in-lammps-kim-query "Link to this heading"){.headerlink}

The *kim query* command performs a web query to retrieve the predictions of an IM set by *kim init* for material properties archived in [OpenKIM](https://openkim.org){.reference .external}, or to obtain a list of available models that support a given set of atomic species.

::::: {#id5 .section}
### Syntax[](#id5 "Link to this heading"){.headerlink}
...
::::::::: {#id6 .section}
### Examples[](#id6 "Link to this heading"){.headerlink}
...
The result of the query is stored in one or more [[string style variables]{.doc}]variable.md){.reference .internal} as determined by the optional *formatarg* argument. For the "list" setting of *formatarg* (or if *formatarg* is not specified), the result is returned as a space-separated list of values in *variable*. The *formatarg* keyword "split" separates the result values into individual variables of the form *prefix_I*, where *prefix* is set to the *kim query* *variable* argument and *I* ranges from 1 to the number of returned values. The number and order of the returned values is determined by the type of query performed. The *formatarg* keyword "index" returns a [[variable style index]{.doc}]variable.md){.reference .internal} that can be incremented via the [[next]{.doc}]next.md){.reference .internal} command. This enables the construction of simple loops over the returned values by the type of query performed.
...
The second required argument *query_function* is the name of the query function to be called (e.g. *get_lattice_constant_cubic*). All following [[arguments]{.doc}]Commands_parse.md){.reference .internal} are parameters handed over to the web query in the format *keyword=value*, where *value* is always an array of one or more comma-separated items in brackets. The list of supported keywords and the type and format of their values depend on the query function used. The current list of query functions is available on the OpenKIM webpage at [https://openkim.org/doc/usage/kim-query](https://openkim.org/doc/usage/kim-query/){.reference .external}.

::: {.admonition .note}
Note

The *get_available_models* function's output is post-processed by LAMMPS so that only the names of models that are currently installed are stored in the variable. Matching models that are not installed are instead written to the LAMMPS log file.
:::
...
Each *query_function* is associated with a default method (implemented as a [KIM Test](https://openkim.org/doc/evaluation/kim-tests/){.reference .external}) used to compute this property. In cases where there are multiple methods in OpenKIM for computing a property, a *method* keyword can be provided to select the method of choice. See the [query documentation](https://openkim.org/doc/usage/kim-query){.reference .external} to see which methods are available for a given *query_function*.
:::
:::::::::

:::::::::::::::::: {#kim-query-usage-examples-and-further-clarifications .section}
### *kim query* Usage Examples and Further Clarifications[](#kim-query-usage-examples-and-further-clarifications "Link to this heading"){.headerlink}
...
The defect formation energy *Eform* is computed by subtracting the ideal fcc cohesive energy of the atoms in the system from *Etot*. The ideal fcc cohesive energy of the atoms is obtained from [OpenKIM](https://openkim.org){.reference .external} for the Ercolessi and Adams (1994) potential.
...
The *kim param get* and *kim param set* commands provide an interface to access and change the parameters of a KIM PM that "publishes" its parameters and makes them publicly available (see the [KIM API documentation](https://kim-api.readthedocs.io/en/devel/features.html){.reference .external} for details).
...
:::::: {#id13 .section}
### Syntax[](#id13 "Link to this heading"){.headerlink}
...
- param_name = name of a KIM portable model parameter (which is published by the PM and available for access). The specific string used to identify a parameter is defined by the PM. For example, for the [Stillinger-Weber (SW) potential in OpenKIM](https://openkim.org/id/SW_StillingerWeber_1985_Si__MO_405512056662_005){.reference .external}, the parameter names are *A, B, p, q, sigma, gamma, cutoff, lambda, costheta0*
...
The result of a *get* operation for a specific *index_range* is stored in one or more [[LAMMPS string style variables]{.doc}]variable.md){.reference .internal} as determined by the optional *formatarg* argument [[documented above.]{.std .std-ref}](#formatarg-options){.reference .internal} If not specified, the default for *formatarg* is "explicit" for the *kim param* command.
...
:::::::::::::::::::::::: {#kim-param-usage-examples-and-further-clarifications .section}
### *kim param* Usage Examples and Further Clarifications[](#kim-param-usage-examples-and-further-clarifications "Link to this heading"){.headerlink}
...
In the above example, elements 7-9 of the lambda parameter correspond to Te-Te, Te-Zm and Te-Se interactions. This can be determined by visiting the [model page for the specified potential](https://openkim.org/id/SW_ZhouWardMartin_2013_CdTeZnSeHgS__MO_503261197030_002){.reference .external} and looking at its parameter file linked to at the bottom of the page (file with .param ending) and consulting the README documentation provided with the driver for the PM being used. A link to the driver is provided at the top of the model page.
:::
...
- property_id = identifier of a [KIM Property Definition](https://openkim.org/properties/){.reference .external}, which can be (1) a property short name, (2) the full unique ID of the property (including the contributor and date), (3) a file name corresponding to a local property definition file

- key_name = one of the keys belonging to the specified KIM property definition

- key_name_key = a key belonging to a key-value pair (standardized in the [KIM Properties Framework](https://openkim.org/doc/schema/properties-framework/){.reference .external})
...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: {#kim-property-usage-examples-and-further-clarifications .section}
### *kim property* Usage Examples and Further Clarifications[](#kim-property-usage-examples-and-further-clarifications "Link to this heading"){.headerlink}
...
Calling this command with a non-positive index, e.g. [`kim`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`property`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`modify`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`2`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`key`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`species`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`source-value`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`0`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Al`{.docutils .literal .notranslate}]{.pre}, or an incorrect number of input arguments, e.g. [`kim`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`property`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`modify`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`2`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`key`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`species`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`source-value`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`1:4`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Al`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Al`{.docutils .literal .notranslate}]{.pre}, will result in an error.

As an example of modifying multidimensional arrays, consider the "basis-atoms" key in the [cohesive-energy-relation-cubic-crystal](https://openkim.org/properties/show/2014-04-15/staff@noreply.openkim.org/cohesive-energy-relation-cubic-crystal){.reference .external} property definition. This is a two-dimensional array containing the fractional coordinates of atoms in the unit cell of the cubic crystal. In the case of, e.g. a conventional fcc unit cell, the "source-value" key in the map associated with this key should be assigned the following value:
...
::: {#citation-of-openkim-ims .section}
## Citation of OpenKIM IMs[](#citation-of-openkim-ims "Link to this heading"){.headerlink}

When publishing results obtained using OpenKIM IMs researchers are requested to cite the OpenKIM project [[(Tadmor)]{.std .std-ref}](#kim-mainpaper){.reference .internal}, KIM API [[(Elliott)]{.std .std-ref}](#id18){.reference .internal}, and the specific IM codes used in the simulations, in addition to the relevant scientific references for the IM. The citation format for an IM is displayed on its page on [OpenKIM](https://openkim.org){.reference .external} along with the corresponding BibTex file, and is automatically added to the LAMMPS citation reminder.
...
The *kim* command is part of the KIM package. It is only enabled if LAMMPS is built with that package. A requirement for the KIM package, is the KIM API library that must be downloaded from the [OpenKIM website](https://openkim.org/kim-api/){.reference .external} and installed before LAMMPS is compiled. When installing LAMMPS from binary, the kim-api package is a dependency that is automatically downloaded and installed. The *kim query* command requires the *libcurl* library to be installed. The *kim property* command requires *Python* 3.6 or later and the *kim-property* python package to be installed. See the KIM section of the [[Packages details]{.doc}]Packages_details.md){.reference .internal} for details.
...
**(Tadmor)** Tadmor, Elliott, Sethna, Miller and Becker, JOM, 63, 17 (2011). doi: [https://doi.org/10.1007/s11837-011-0102-6](https://doi.org/10.1007/s11837-011-0102-6){.reference .external}

**(Elliott)** Elliott, Tadmor and Bernstein, [https://openkim.org/kim-api/](https://openkim.org/kim-api/){.reference .external} (2011) doi: [https://doi.org/10.25950/FF8F563A](https://doi.org/10.25950/FF8F563A){.reference .external}
:::
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
