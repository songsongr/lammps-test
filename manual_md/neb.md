...
# neb command[](#neb-command "Link to this heading"){.headerlink}
...
NEB is a method for finding both the atomic configurations and height of the energy barrier associated with a transition state, e.g. for an atom to perform a diffusive hop from one energy basin to another in a coordinated fashion with its neighbors. The implementation in LAMMPS follows the discussion in these 4 papers: [[(HenkelmanA)]{.std .std-ref}](#henkelmana){.reference .internal}, [[(HenkelmanB)]{.std .std-ref}](#henkelmanb){.reference .internal}, [[(Nakano)]{.std .std-ref}](#nakano3){.reference .internal} and [[(Maras)]{.std .std-ref}](#maras2){.reference .internal}.

Each replica runs on a partition of one or more processors. Processor partitions are defined at run-time using the [[-partition command-line switch]{.doc}]Run_options.md){.reference .internal}. Note that if you have MPI installed, you can run a multi-replica simulation with more replicas (partitions) than you have physical processors, e.g you can run a 10-replica simulation on just one or two processors. You will simply not get the performance speed-up you would see with one or more physical processors per replica. See the [[Howto replica]{.doc}]Howto_replica.md){.reference .internal} doc page for further discussion.

::: {.admonition .note}
Note

As explained below, a NEB calculation performs a damped dynamics minimization across all the replicas. The minimizer uses whatever timestep you have defined in your input script, via the [[timestep]{.doc}]timestep.md){.reference .internal} command. Often NEB will converge more quickly if you use a timestep about 10x larger than you would normally use for dynamics simulations.
:::
...
which in this case will substitute the partition ID (0 to N-1) for the variable I, which is also effectively the replica ID. See the [[variable]{.doc}]variable.md){.reference .internal} command for other options, such as using world-, universe-, or uloop-style variables.
...
For a *file-style* setting of *none*, no filename is specified. Each replica is assumed to already be in its initial configuration at the time the neb command is issued. This allows each replica to define its own configuration by reading a replica-specific data or restart or dump file, via the [[read_data]{.doc}]read_data.md){.reference .internal}, [[read_restart]{.doc}]read_restart.md){.reference .internal}, or [[read_dump]{.doc}]read_dump.md){.reference .internal} commands. The replica-specific names of these files can be specified as in the discussion above for the *each* file-style. Also see the section below for how a NEB calculation can produce restart files, so that a long calculation can be restarted if needed.
...
A NEB calculation proceeds in two stages, each of which is a minimization procedure, performed via damped dynamics. To enable this, you must first define a damped dynamics [[min_style]{.doc}]min_style.md){.reference .internal}, such as *quickmin* or *fire*. The *cg*, *sd*, and *hftn* styles cannot be used, since they perform iterative line searches in their inner loop, which cannot be easily synchronized across multiple replicas.

The minimizer tolerances for energy and force are set by *etol* and *ftol*, the same as for the [[minimize]{.doc}]minimize.md){.reference .internal} command.
...
An atom map must be defined which it is not by default for [[atom_style atomic]{.doc}]atom_style.md){.reference .internal} problems. The [[atom_modify map]{.doc}]atom_modify.md){.reference .internal} command can be used to do this.

The minimizers in LAMMPS operate on all atoms in your system, even non-NEB atoms, as defined above. To prevent non-NEB atoms from moving during the minimization, you should use the [[fix setforce]{.doc}]fix_setforce.md){.reference .internal} command to set the force on each of those atoms to 0.0. This is not required, and may not even be desired in some cases, but if those atoms move too far (e.g. because the initial state of your system was not well-minimized), it can cause problems for the NEB procedure.

The damped dynamics [[minimizers]{.doc}]min_style.md){.reference .internal}, such as *quickmin* and *fire*), adjust the position and velocity of the atoms via an Euler integration step. Thus you must define an appropriate [[timestep]{.doc}]timestep.md){.reference .internal} to use with NEB. As mentioned above, NEB will often converge more quickly if you use a timestep about 10x larger than you would normally use for dynamics simulations.
...
The file can be ASCII text or a compressed text file (detected by a [[supported compression format suffix]{.std .std-ref}]Build_settings.md#gzip){.reference .internal}). The file can contain initial blank lines or comment lines starting with "#" which are ignored. The first non-blank, non-comment line should list N = the number of lines to follow. The N successive lines contain the following information:
...
Also note there is no requirement that the atoms in the file correspond to the NEB atoms in the group defined by the [[fix neb]{.doc}]fix_neb.md){.reference .internal} command. Not every NEB atom need be in the file, and non-NEB atoms can be listed in the file.
...
The "path angle" (pathangle) for the replica i which is the angle between the 3N-length vectors [\\((R\_{i-1} - R_i)\\)]{.math .notranslate .nohighlight} and [\\((R\_{i+1} - R_i)\\)]{.math .notranslate .nohighlight} (where [\\(R_i\\)]{.math .notranslate .nohighlight} is the atomic coordinates of replica *i*). A "path angle" of 180 indicates that replicas *i*-1, *i* and *i*+1 are aligned. "angletangrad" is the angle between the 3N-length tangent vector and the 3N-length force vector at image *i*. The tangent vector is calculated as in [[(HenkelmanA)]{.std .std-ref}](#henkelmana){.reference .internal} for all intermediate replicas and at R2 - R1 and RM - RM-1 for the first and last replica, respectively. "anglegrad" is the angle between the 3N-length energy gradient vector of replica *i* and that of replica *i*+1. It is not defined for the final replica and reads nan. gradV is the norm of the energy gradient of image *i* ([\\(\\nabla V\\)]{.math .notranslate .nohighlight}). ReplicaForce is the two-norm of the 3N-length force vector (including nudging forces) for replica *i*. MaxAtomForce is the maximum force component of any atom in replica *i*.

When a NEB calculation does not converge properly, the supplementary information can help understanding what is going wrong. For instance when the path angle becomes acute, the definition of tangent used in the NEB calculation is questionable and the NEB cannot may diverge [[(Maras)]{.std .std-ref}](#maras2){.reference .internal}.
...
If [[dump]{.doc}]dump.md){.reference .internal} commands in the input script define a filename that includes a *universe* or *uloop* style [[variable]{.doc}]variable.md){.reference .internal}, then one dump file (per dump command) will be created for each replica. At the end of the NEB calculation, the final snapshot in each file will contain the sequence of snapshots that transition the system over the energy barrier. Earlier snapshots will show the convergence of the replicas to the MEP.

Likewise, [[restart]{.doc}]restart.md){.reference .internal} filenames can be specified with a *universe* or *uloop* style [[variable]{.doc}]variable.md){.reference .internal}, to generate restart files for each replica. These may be useful if the NEB calculation fails to converge properly to the MEP, and you wish to restart the calculation from an intermediate point with altered parameters.
...
[![neb1](_images/hop1.jpg){style="width: 48%;"}](_images/hop1.jpg){.reference .internal} [![neb2](_images/hop2.jpg){style="width: 48%;"}](_images/hop2.jpg){.reference .internal}
...
To read compressed files, you must compile LAMMPS with the [`-DLAMMPS_GZIP`{.docutils .literal .notranslate}]{.pre} option. See the [[Build settings]{.doc}]Build_settings.md){.reference .internal} doc page for details.
:::
...
[[prd]{.doc}]prd.md){.reference .internal}, [[temper]{.doc}]temper.md){.reference .internal}, [[fix langevin]{.doc}]fix_langevin.md){.reference .internal}, [[fix viscous]{.doc}]fix_viscous.md){.reference .internal}, [[fix neb]{.doc}]fix_neb.md){.reference .internal}
:::
