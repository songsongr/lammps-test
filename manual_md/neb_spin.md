...
:::::::::::::::::::::: {#neb-spin-command .section}
[]{#index-0}

# neb/spin command[](#neb-spin-command "Link to this heading"){.headerlink}
...
As explained below, a GNEB calculation performs a minimization across all the replicas. One of the [[spin]{.doc}]min_spin.md){.reference .internal} style minimizers has to be defined in your input script.
:::
...
The "angular distance" between the starting and final point is evaluated in the geodesic sense, as described in [[(Bessarab)]{.std .std-ref}](#bessaraba){.reference .internal}.
:::
...
where [\\(\\vec{m}\_i\^I\\)]{.math .notranslate .nohighlight} is the initial spin configuration for spin i, [\\(\\omega_i\^{\\nu}\\)]{.math .notranslate .nohighlight} is a rotation angle defined as:

::: {.math .notranslate .nohighlight}
\\\[\\omega_i\^{\\nu} = (\\nu - 1) \\Delta \\omega_i \\mathrm{\~\~and\~\~} \\Delta \\omega_i = \\frac{\\omega_i}{Q-1}\\\]
:::

with [\\(\\nu\\)]{.math .notranslate .nohighlight} the image number, Q the total number of images, and [\\(\\omega_i\\)]{.math .notranslate .nohighlight} the total rotation between the initial and final spins. [\\(\\vec{k}\_i\\)]{.math .notranslate .nohighlight} defines a rotation axis such as:
...
For a *file-style* setting of *each*, a filename is specified which is assumed to be unique to each replica. See the [[neb]{.doc}]neb.md){.reference .internal} documentation page for more information about this option.
...
A NEB calculation proceeds in two stages, each of which is a minimization procedure. To enable this, you must first define a [[min_style]{.doc}]min_style.md){.reference .internal}, using either the *spin*, *spin/cg*, or *spin/lbfgs* style (see [[min_spin]{.doc}]min_spin.md){.reference .internal} for more information). The other styles cannot be used, since they relax the lattice degrees of freedom instead of the spins.

The minimizer tolerances for energy and force are set by *etol* and *ttol*, the same as for the [[minimize]{.doc}]minimize.md){.reference .internal} command.
...
In the second stage of GNEB, the replica with the highest energy is selected and the inter-replica forces on it are converted to a force that drives its spin coordinates to the top or saddle point of the barrier, via the barrier-climbing calculation described in [[(Bessarab)]{.std .std-ref}](#bessaraba){.reference .internal}. As before, the other replicas rearrange themselves along the MEP so as to be roughly equally spaced.
...
Also note there is no requirement that the atoms in the file correspond to the GNEB atoms in the group defined by the [[fix neb]{.doc}]fix_neb.md){.reference .internal} command. Not every GNEB atom need be in the file, and non-GNEB atoms can be listed in the file.
...
A c file script in provided in the tool/spin/interpolate_gneb directory, that interpolates the MEP given the information provided by the *verbose* output option (as detailed in Appendix D of [[(Bessarab)]{.std .std-ref}](#bessaraba){.reference .internal}).
::::::::::::
...
[[min/spin]{.doc}]min_spin.md){.reference .internal}, [[fix neb/spin]{.doc}]fix_neb_spin.md){.reference .internal}
:::
