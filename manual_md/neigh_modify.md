...
::::::::::::::::: {#neigh-modify-command .section}
[]{#index-0}

# neigh_modify command[](#neigh-modify-command "Link to this heading"){.headerlink}
...
- When a large collection of atoms is treated as frozen, interactions between those atoms can be turned off to save needless computation. E.g. Using the [[fix setforce]{.doc}]fix_setforce.md){.reference .internal} command to freeze a wall or portion of a bio-molecule.

- When one or more rigid bodies are specified, interactions within each body can be turned off to save needless computation. See the [[fix rigid]{.doc}]fix_rigid.md){.reference .internal} command for more details.

::: versionchanged
[Changed in version 29Aug2024: ]{.versionmodified .changed}Support for type labels was added.
:::
...
Note that the exclude options only affect pairwise interactions; see the [[delete_bonds]{.doc}]delete_bonds.md){.reference .internal} command for information on turning off bond interactions.

::: {.admonition .note}
Note

Excluding pairwise interactions will not work correctly when also using a long-range solver via the [[kspace_style]{.doc}]kspace_style.md){.reference .internal} command. LAMMPS will give a warning to this effect. This is because the short-range pairwise interaction needs to subtract off a term from the total energy for pairs whose short-range interaction is excluded, to compensate for how the long-range solver treats the interaction. This is done correctly for pairwise interactions that are excluded (or weighted) via the [[special_bonds]{.doc}]special_bonds.md){.reference .internal} command. But it is not done for interactions that are excluded via these neigh_modify exclude options.
:::
...
[[neighbor]{.doc}]neighbor.md){.reference .internal}, [[delete_bonds]{.doc}]delete_bonds.md){.reference .internal}
:::
