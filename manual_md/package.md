...
# package command[](#package-command "Link to this heading"){.headerlink}

- [Syntax](#syntax){#id1 .reference .internal}

- [Examples](#examples){#id2 .reference .internal}

- [Description](#description){#id3 .reference .internal}

  - [GPU package settings](#gpu-package-settings){#id4 .reference .internal}

  - [INTEL package settings](#intel-package-settings){#id5 .reference .internal}

  - [KOKKOS package settings](#kokkos-package-settings){#id6 .reference .internal}

  - [OPENMP package settings](#openmp-package-settings){#id7 .reference .internal}

- [Restrictions](#restrictions){#id8 .reference .internal}

- [Related commands](#related-commands){#id9 .reference .internal}

- [Defaults](#defaults){#id10 .reference .internal}

::::: {#syntax .section}
## [Syntax](#contents){.toc-backref role="doc-backlink"}[](#syntax "Link to this heading"){.headerlink}
...
::::: {#examples .section}
## [Examples](#contents){.toc-backref role="doc-backlink"}[](#examples "Link to this heading"){.headerlink}
...
:::::::::::::::: {#description .section}
## [Description](#contents){.toc-backref role="doc-backlink"}[](#description "Link to this heading"){.headerlink}
...
This command can also be specified from the command-line when launching LAMMPS, using the "-pk" [[command-line switch]{.doc}]Run_options.md){.reference .internal}. The syntax is exactly the same as when used in an input script.
...
The KOKKOS package requires a "-k on" [[command-line switch]{.doc}]Run_options.md){.reference .internal} respectively, which invokes a "package kokkos" command with default settings.

For the GPU, INTEL, and OPENMP packages, if a "-sf gpu" or "-sf intel" or "-sf omp" [[command-line switch]{.doc}]Run_options.md){.reference .internal} is used to auto-append accelerator suffixes to various styles in the input script, then those switches also invoke a "package gpu", "package intel", or "package omp" command with default settings.

::: {.admonition .note}
Note

A package command for a particular style can be invoked multiple times when a simulation is setup, e.g. by the [[-c on, -k on, -sf, and -pk command-line switches]{.doc}]Run_options.md){.reference .internal}, and by using this command in an input script. Each time it is used all of the style options are set, either to default values or to specified settings. I.e. settings from previous invocations do not persist across multiple invocations.
:::

See the [[Accelerator packages]{.doc}]Speed_packages.md){.reference .internal} page for more details about using the various accelerator packages for speeding up LAMMPS simulations.

:::::: {#gpu-package-settings .section}
### [GPU package settings](#contents){.toc-backref role="doc-backlink"}[](#gpu-package-settings "Link to this heading"){.headerlink}

The *gpu* style invokes settings associated with the use of the [[GPU package]{.std .std-ref}]Packages_details.md#pkg-gpu){.reference .internal}.
...
::: versionchanged
[Changed in version 10Dec2025: ]{.versionmodified .changed}Updated description to the current state of the GPU package
:::
...
The *split* keyword can be used for load balancing force calculations between CPU and GPU cores in GPU-enabled pair styles. If 0 \< *split* \< 1.0, a fixed fraction of particles is offloaded to the GPU while force calculation for the other particles occurs simultaneously on the CPU. If *split* \< 0.0, the optimal fraction (based on CPU and GPU timings) is calculated every 25 timesteps, i.e. dynamic load-balancing across the CPU and GPU is performed. If *split* = 1.0, all force calculations for GPU accelerated pair styles are performed on the GPU. In this case, other [[hybrid]{.doc}]pair_hybrid.md){.reference .internal} pair interactions, [[bond]{.doc}]bond_style.md){.reference .internal}, [[angle]{.doc}]angle_style.md){.reference .internal}, [[dihedral]{.doc}]dihedral_style.md){.reference .internal}, [[improper]{.doc}]improper_style.md){.reference .internal}, and [[long-range]{.doc}]kspace_style.md){.reference .internal} calculations can be performed on the CPU while the GPU is performing force calculations for the GPU-enabled pair style. If all CPU force computations complete before the GPU completes, LAMMPS will block until the GPU has finished before continuing the timestep.
...
CONFIG_ID can be 0. SHUFFLE_AVAIL in {0,1} indicates that inline-PTX (NVIDIA) or OpenCL extensions (Intel) should be used for horizontal vector operations. FAST_MATH in {0,1} indicates that OpenCL fast math optimizations are used during the build and hardware-accelerated transcendental functions are used when available. THREADS_PER\_\* give the default *tpa* values for ellipsoidal models, styles using charge, and any other styles. The BLOCK\_\* parameters specify the block sizes for various kernel calls and the MAX\_\*SHARED\_\* parameters are used to determine the amount of local shared memory to use for storing model parameters.
...
:::: {#intel-package-settings .section}
### [INTEL package settings](#contents){.toc-backref role="doc-backlink"}[](#intel-package-settings "Link to this heading"){.headerlink}
...
:::: {#kokkos-package-settings .section}
### [KOKKOS package settings](#contents){.toc-backref role="doc-backlink"}[](#kokkos-package-settings "Link to this heading"){.headerlink}
...
A value of *full* uses a full neighbor lists and is the default when running on GPUs. This performs twice as much computation as the *half* option, however that is often a win because it is thread-safe and does not require atomic operations in the calculation of pair forces. For that reason, *full* is the default setting for GPUs. However, when running on CPUs, a *half* neighbor list is the default because it are often faster, just as it is for non-accelerated pair styles. Similarly, the *neigh/qeq* keyword determines how neighbor lists are built for [[fix qeq/reaxff/kk]{.doc}]fix_qeq_reaxff.md){.reference .internal}.

If the *neigh/thread* keyword is set to *off*, then the KOKKOS package threads only over atoms. However, for small systems, this may not expose enough parallelism to keep a GPU busy. When this keyword is set to *on*, the KOKKOS package threads over both atoms and neighbors of atoms. When using *neigh/thread* *on*, the [[newton pair]{.doc}]newton.md){.reference .internal} setting must be "off". Using *neigh/thread* *on* may be slower for large systems, so this this option is turned on by default only when running on one or more GPUs and there are 16k atoms or less owned by an MPI rank. Not all KOKKOS-enabled potentials support this keyword yet, and only thread over atoms. Many simple pairwise potentials such as Lennard-Jones do support threading over both atoms and neighbors.
...
The *newton* keyword sets the Newton flags for pairwise and bonded interactions to *off* or *on*, the same as the [[newton]{.doc}]newton.md){.reference .internal} command allows. The default for GPUs is *off* because this will almost always give better performance for the KOKKOS package. This means more computation is done, but less communication. However, when running on CPUs a value of *on* is the default since it can often be faster, just as it is for non-accelerated pair styles

The *binsize* keyword sets the size of bins used to bin atoms during neighbor list builds. The same value can be set by the [[neigh_modify binsize]{.doc}]neigh_modify.md){.reference .internal} command. Making it an option in the package kokkos command allows it to be set from the command-line. The default value for CPUs is 0.0, which means the LAMMPS default will be used, which is bins = 1/2 the size of the pairwise cutoff + neighbor skin distance. This is fine when neighbor lists are built on the CPU. For GPU builds, a 2x larger binsize equal to the pairwise cutoff + neighbor skin is often faster, which is the default. Note that if you use a longer-than-usual pairwise cutoff, e.g. to allow for a smaller fraction of KSpace work with a [[long-range Coulombic solver]{.doc}]kspace_style.md){.reference .internal} because the GPU is faster at performing pairwise interactions, then this rule of thumb may give too large a binsize and the default should be overridden with a smaller value.
...
The *sort* keyword determines whether the host or device performs atom sorting, see the [[atom_modify sort]{.doc}]atom_modify.md){.reference .internal} command. The value options for the *sort* keyword are *no* or *device* similar to the *comm* keywords above. If a value of *host* is used it will be automatically be changed to *no* since the *sort* keyword does not support *host* mode. Not all fix styles with extra atom data support *device* mode and in that case a warning will be given and atom sorting will run in *no* mode instead.

::: versionadded
[Added in version 17Apr2024.]{.versionmodified .added}
:::

The *atom/map* keyword determines whether the host or device builds the atom_map, see the [[atom_modify map]{.doc}]atom_modify.md){.reference .internal} command. The value options for the *atom/map* keyword are identical to the *sort* keyword above.
...
:::::: {#openmp-package-settings .section}
### [OPENMP package settings](#contents){.toc-backref role="doc-backlink"}[](#openmp-package-settings "Link to this heading"){.headerlink}
...
If you build LAMMPS with the GPU, INTEL, and / or OPENMP packages, be aware these packages all allow setting of the *Nthreads* value via their package commands, but there is only a single global *Nthreads* value used by OpenMP. Thus if multiple package commands are invoked, you should ensure the values are consistent. If they are not, the last one invoked will take precedence, for all packages. Also note that if the [[-sf hybrid intel omp command-line switch]{.doc}]Run_options.md){.reference .internal} is used, it invokes a "package intel" command, followed by a "package omp" command, both with a setting of *Nthreads* = 0. Likewise for a hybrid suffix for gpu and omp. Note that KOKKOS also supports setting the number of OpenMP threads from the command-line using the "-k on" [[command-line switch]{.doc}]Run_options.md){.reference .internal}. The default for KOKKOS is 1 thread per MPI task, so any other number of threads should be explicitly set using the "-k on" command-line switch (and this setting should be consistent with settings from any other packages used).
:::
...
## [Restrictions](#contents){.toc-backref role="doc-backlink"}[](#restrictions "Link to this heading"){.headerlink}
...
The *gpu* style of this command can only be invoked if LAMMPS was built with the GPU package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} doc page for more info.

The *intel* style of this command can only be invoked if LAMMPS was built with the INTEL package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} page for more info.

The *kokkos* style of this command can only be invoked if LAMMPS was built with the KOKKOS package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} doc page for more info.

The *omp* style of this command can only be invoked if LAMMPS was built with the OPENMP package. See the [[Build package]{.doc}]Build_package.md){.reference .internal} doc page for more info.
:::

::: {#related-commands .section}
## [Related commands](#contents){.toc-backref role="doc-backlink"}[](#related-commands "Link to this heading"){.headerlink}

[[suffix]{.doc}]suffix.md){.reference .internal}, [[-pk command-line switch]{.doc}]Run_options.md){.reference .internal}
:::

::::::::::::: {#defaults .section}
## [Defaults](#contents){.toc-backref role="doc-backlink"}[](#defaults "Link to this heading"){.headerlink}
...
These settings are made automatically if the "-sf gpu" [[command-line switch]{.doc}]Run_options.md){.reference .internal} is used. If it is not used, you must invoke the package gpu command in your input script or via the "-pk gpu" [[command-line switch]{.doc}]Run_options.md){.reference .internal}.
...
The default ghost option is determined by the pair style being used. This value is output to the screen in the offload report at the end of each run. Note that all of these settings, except "omp" and "mode", are ignored if LAMMPS was not built with Xeon Phi co-processor support. These settings are made automatically if the "-sf intel" [[command-line switch]{.doc}]Run_options.md){.reference .internal} is used. If it is not used, you must invoke the package intel command in your input script or via the "-pk intel" [[command-line switch]{.doc}]Run_options.md){.reference .internal}.
...
These settings are made automatically by the required "-k on" [[command-line switch]{.doc}]Run_options.md){.reference .internal}. You can change them by using the package kokkos command in your input script or via the [[-pk kokkos command-line switch]{.doc}]Run_options.md){.reference .internal}.
...
These settings are made automatically if the "-sf omp" [[command-line switch]{.doc}]Run_options.md){.reference .internal} is used. If it is not used, you must invoke the package omp command in your input script or via the "-pk omp" [[command-line switch]{.doc}]Run_options.md){.reference .internal}.
:::::::::::::
::::::::::::::::::::::::::::::::::::
:::::::::::::::::::::::::::::::::::::
::::::::::::::::::::::::::::::::::::::
