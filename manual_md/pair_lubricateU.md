...
::: {#pair-style-lubricateu-command .section}
[]{#index-1}[]{#index-0}

# pair_style lubricateU command[](#pair-style-lubricateu-command "Link to this heading"){.headerlink}
:::

::::::::::::::::: {#pair-style-lubricateu-poly-command .section}
# pair_style lubricateU/poly command[](#pair-style-lubricateu-poly-command "Link to this heading"){.headerlink}
...
The interactions have 2 components. The first is Ball-Melrose lubrication terms via the formulas in [[(Ball and Melrose)]{.std .std-ref}](#ball2){.reference .internal}
...
The other component is due to the Fast Lubrication Dynamics (FLD) approximation, described in [[(Kumar)]{.std .std-ref}](#kumar2){.reference .internal}. The equation being solved to balance the forces and torques is

::: {.math .notranslate .nohighlight}
\\\[-R\_{FU}(U-U\^{\\infty}) = -R\_{FE}E\^{\\infty} - F\^{rest}\\\]
:::

where U represents the velocities and angular velocities of the particles, [\\(U\^{\\infty}\\)]{.math .notranslate .nohighlight} represents the velocities and the angular velocities of the undisturbed fluid, and [\\(E\^{\\infty}\\)]{.math .notranslate .nohighlight} represents the rate of strain tensor of the undisturbed fluid flow with viscosity *mu*. Again, note that this is dynamic viscosity which has units of mass/distance/time, not kinematic viscosity. Volume fraction corrections to R_FU are included if *flagVF* is set to 1 (default).
...
When using these styles, the pair styles are designed to be used with implicit time integration and a correspondingly larger timestep. Thus either [[fix nve/noforce]{.doc}]fix_nve_noforce.md){.reference .internal} should be used for spherical particles defined via [[atom_style sphere]{.doc}]atom_style.md){.reference .internal} or [[fix nve/asphere/noforce]{.doc}]fix_nve_asphere_noforce.md){.reference .internal} should be used for spherical particles defined via [[atom_style ellipsoid]{.doc}]atom_style.md){.reference .internal}. This is because the velocity and angular momentum of each particle is set by the pair style, and should not be reset by the time integration fix.
:::
...
These pair styles can only be used via the *pair* keyword of the [[run_style respa]{.doc}]run_style.md){.reference .internal} command. They does not support the *inner*, *middle*, *outer* keywords.
:::
...
For sheared suspensions, it is assumed that the shearing is done in the xy plane, with x being the velocity direction and y being the velocity-gradient direction. In this case, one must use [[fix deform]{.doc}]fix_deform.md){.reference .internal} with the same rate of shear (erate).
...
[[pair_coeff]{.doc}]pair_coeff.md){.reference .internal}, [[pair_style lubricate]{.doc}]pair_lubricate.md){.reference .internal}
:::
