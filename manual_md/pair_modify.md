...
:::::::::::::::::::: {#pair-modify-command .section}
[]{#index-0}

# pair_modify command[](#pair-modify-command "Link to this heading"){.headerlink}
...
Modify the parameters of the currently defined pair style. If the pair style is [[hybrid or hybrid/overlay]{.doc}]pair_hybrid.md){.reference .internal}, then the specified parameters are by default modified for all the hybrid sub-styles.
...
The *table* and *table/disp* keywords apply to pair styles with a long-range Coulombic term or long-range dispersion term respectively; see the page for individual styles to see which potentials support these options. If N is non-zero, a table of length 2\^N is pre-computed for forces and energies, which can shrink their computational cost by up to a factor of 2. The table is indexed via a bit-mapping technique [[(Wolff)]{.std .std-ref}](#wolff1){.reference .internal} and a linear interpolation is performed between adjacent table values. In our experiments with different table styles (lookup, linear, spline), this method typically gave the best performance in terms of speed and accuracy.
...
When the *tail* keyword is set to *yes*, certain pair styles will add a long-range VanderWaals tail "correction" to the energy and pressure. These corrections are bookkeeping terms which do not affect dynamics, unless a constant-pressure simulation is being performed. See the page for individual styles to see which support this option. These corrections are included in the calculation and printing of thermodynamic quantities (see the [[thermo_style]{.doc}]thermo_style.md){.reference .internal} command). Their effect will also be included in constant NPT or NPH simulations where the pressure influences the simulation box dimensions (e.g. the [[fix npt]{.doc}]fix_nh.md){.reference .internal} and [[fix nph]{.doc}]fix_nh.md){.reference .internal} commands). The formulas used for the long-range corrections come from equation 5 of [[(Sun)]{.std .std-ref}](#sun){.reference .internal}.

::: {.admonition .note}
Note

The tail correction terms are computed at the beginning of each run, using the current atom counts of each atom type. If atoms are deleted (or lost) or created during a simulation, e.g. via the [[fix gcmc]{.doc}]fix_gcmc.md){.reference .internal} command, the correction factors are not re-computed. If you expect the counts to change dramatically, you can break a run into a series of shorter runs so that the correction factors are re-computed more frequently.
:::
...
The *compute* keyword allows pairwise computations to be turned off, even though a [[pair_style]{.doc}]pair_style.md){.reference .internal} is defined. This is not useful for running a real simulation, but can be useful for debugging purposes or for performing a [[rerun]{.doc}]rerun.md){.reference .internal} simulation, when you only wish to compute partial forces that do not include the pairwise contribution.

Two examples are as follows. First, this option allows you to perform a simulation with [[pair_style hybrid]{.doc}]pair_hybrid.md){.reference .internal} with only a subset of the hybrid sub-styles enabled. Second, this option allows you to perform a simulation with only long-range interactions but no short-range pairwise interactions. Doing this by simply not defining a pair style will not work, because the [[kspace_style]{.doc}]kspace_style.md){.reference .internal} command requires a Kspace-compatible pair style be defined.
...
The *pair* keyword can only be used with the [[hybrid and hybrid/overlay]{.doc}]pair_hybrid.md){.reference .internal} pair styles. If used, it must appear first in the list of keywords.

Its meaning is that all the following parameters will only be modified for the specified sub-style. If the sub-style is defined multiple times, then an additional numeric argument *N* must also be specified, which is a number from 1 to M where M is the number of times the sub-style was listed in the [[pair_style hybrid]{.doc}]pair_hybrid.md){.reference .internal} command. The extra number indicates which instance of the sub-style the remaining keywords will be applied to.
...
The *special* keyword overrides the global [[special_bonds]{.doc}]special_bonds.md){.reference .internal} 1-2, 1-3, 1-4 exclusion settings (weights) for the sub-style selected by the *pair* keyword.

Similar to the [[special_bonds]{.doc}]special_bonds.md){.reference .internal} command, it takes 4 arguments. The *which* argument can be *lj* to change only the non-Coulomb weights (e.g. Lennard-Jones or Buckingham), *coul* to change only the Coulombic settings, or *lj/coul* to change both to the same values. The *wt1,wt2,wt3* values are numeric weights from 0.0 to 1.0 inclusive, for the 1-2, 1-3, and 1-4 bond topology neighbors, respectively. The *special* keyword can be used multiple times, e.g. to set the *lj* and *coul* settings to different values.
...
Weights of exactly 0.0 or 1.0 in the [[special_bonds]{.doc}]special_bonds.md){.reference .internal} command have implications on the neighbor list construction, which means that they cannot be overridden by using the *special* keyword. One workaround for this restriction is to use the [[special_bonds]{.doc}]special_bonds.md){.reference .internal} command with weights like 1.0e-10 or 0.999999999 instead of 0.0 or 1.0, respectively, which enables to reset each them to any value between 0.0 and 1.0 inclusively. Otherwise you can set **all** global weights to an arbitrary number between 0.0 or 1.0, like 0.5, and then you have to override **all** *special* settings for **all** sub-styles which use the 1-2, 1-3, and 1-4 exclusion weights in their force/energy computation.
:::

The *compute/tally* keyword disables or enables registering [[compute \*/tally]{.doc}]compute_tally.md){.reference .internal} computes for the sub-style specified by the *pair* keyword. Use *no* to disable, or *yes* to enable.
...
[[pair_style]{.doc}]pair_style.md){.reference .internal}, [[pair_style hybrid]{.doc}]pair_hybrid.md){.reference .internal}, [[pair_coeff]{.doc}]pair_coeff.md){.reference .internal}, [[thermo_style]{.doc}]thermo_style.md){.reference .internal}, [[compute \*/tally]{.doc}]compute_tally.md){.reference .internal}, [[neighbor multi]{.doc}]neighbor.md){.reference .internal}
:::
