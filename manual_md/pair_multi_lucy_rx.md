...
:::::::::::::::::: {#pair-style-multi-lucy-rx-command .section}
[]{#index-1}[]{#index-0}

# pair_style multi/lucy/rx command[](#pair-style-multi-lucy-rx-command "Link to this heading"){.headerlink}
...
Style *multi/lucy/rx* is used in reaction DPD simulations, where the coarse-grained (CG) particles are composed of *m* species whose reaction rate kinetics are determined from a set of *n* reaction rate equations through the [[fix rx]{.doc}]fix_rx.md){.reference .internal} command. The species of one CG particle can interact with a species in a neighboring CG particle through a site-site interaction potential model. Style *multi/lucy/rx* computes the site-site density-dependent force following from the many-body form described in [[(Moore)]{.std .std-ref}](#moore2){.reference .internal} and [[(Warren)]{.std .std-ref}](#warren2){.reference .internal} as
...
See the supporting information of [[(Brennan)]{.std .std-ref}](#brennan2){.reference .internal} or the publication by [[(Moore)]{.std .std-ref}](#moore2){.reference .internal} for more details on the functional form.
