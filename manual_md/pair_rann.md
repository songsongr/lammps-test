...
::::::::::::::::::::::::::::::::::::::::::::::::: {#pair-style-rann-command .section}
[]{#index-0}

# pair_style rann command[](#pair-style-rann-command "Link to this heading"){.headerlink}
...
Pair style *rann* computes pairwise interactions for a variety of materials using rapid atomistic neural network (RANN) potentials ([[Dickel]{.std .std-ref}](#dickel){.reference .internal} , [[Nitol]{.std .std-ref}](#nitol){.reference .internal}). Neural network potentials work by first generating a series of symmetry functions i.e. structural fingerprints from the neighbor list and then using these values as the input layer of a neural network. There is a single output neuron in the final layer which is the energy. Atomic forces are found by analytical derivatives computed via back-propagation. For alloy systems, each element has a unique network.
:::

::::::::::::::::::::::::: {#potential-file-syntax .section}
## Potential file syntax[](#potential-file-syntax "Link to this heading"){.headerlink}
...
The following fingerprint styles are currently defined. See the [[formulation section]{.std .std-ref}](#fingerprints){.reference .internal} below for their definitions:
...
*bias* specifies the bias for a given element and layer. Bias cannot be specified for the output layer. The bias of layer i is a nx1 vector where n is the layer size of i+1:
...
The following activation styles are currently specified. See the [[formulation section]{.std .std-ref}](#activations){.reference .internal} below for their definitions.
...
:::::::::::::::: {#formulation .section}
## Formulation[](#formulation "Link to this heading"){.headerlink}

In the RANN formulation, the total energy of a system of atoms is given by:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}E = \\sum\_{\\alpha} E\^{\\alpha}\\\\\\\\ E\^{\\alpha} = {}\^{N}\\!A\^{\\alpha}\\\\\\\\ {}\^{n+1}\\!A_i\^{\\alpha} = {}\^{n}\\!F\\left({}\^{n}\\!W\_{ij}{\\;}\^{n}\\!A_j\^{\\alpha}+{}\^{n}\\!B_i\\right)\\\\\\\\ {}\^{0}\\!A_i\^{\\alpha} = \\left\[\\begin{array}{c} {}\^1\\!S\\!f\^\\alpha\\\\ {}\^2\\!S\\!f\^\\alpha \\\\\...\\\\\\end{array}\\right\]\\end{split}\\\]
:::

Here [\\(E\^\\alpha\\)]{.math .notranslate .nohighlight} is the energy of atom [\\(\\alpha\\)]{.math .notranslate .nohighlight}, [\\({}\^n\\!F()\\)]{.math .notranslate .nohighlight}, [\\({}\^n\\!W\_{ij}\\)]{.math .notranslate .nohighlight} and [\\({}\^n\\!B_i\\)]{.math .notranslate .nohighlight} are the activation function, weight matrix and bias vector of the n-th layer respectively. The inputs to the first layer are a collection of structural fingerprints which are collected and reshaped into a single long vector. The individual fingerprints may be defined in any order and have various shapes and sizes. Multiple fingerprints of the same type and varying parameters may also be defined in the input layer.

Eight types of structural fingerprints are currently defined. In the following, [\\(\\beta\\)]{.math .notranslate .nohighlight} and [\\(\\gamma\\)]{.math .notranslate .nohighlight} span the full neighbor list of atom [\\(\\alpha\\)]{.math .notranslate .nohighlight}. [\\(\\delta_i\\)]{.math .notranslate .nohighlight} are decay meta-parameters, and [\\(r_e\\)]{.math .notranslate .nohighlight} is a meta-parameter roughly proportional to the first neighbor distance. [\\(r_c\\)]{.math .notranslate .nohighlight} and [\\(dr\\)]{.math .notranslate .nohighlight} are the neighbor cutoff distance and cutoff smoothing distance respectively. [\\(S\^{\\alpha\\beta}\\)]{.math .notranslate .nohighlight} is the MEAM screening function [[(Baskes)]{.std .std-ref}](#baskes97){.reference .internal}, [\\(s_i\^\\alpha\\)]{.math .notranslate .nohighlight} and [\\(s_i\^\\beta\\)]{.math .notranslate .nohighlight} are the atom spin vectors [[(Tranchida)]{.std .std-ref}](#tranchida7){.reference .internal}. [\\(r\^{\\alpha\\beta}\\)]{.math .notranslate .nohighlight} is the distance from atom [\\(\\alpha\\)]{.math .notranslate .nohighlight} to atom [\\(\\beta\\)]{.math .notranslate .nohighlight}, and [\\(\\theta\^{\\alpha\\beta\\gamma}\\)]{.math .notranslate .nohighlight} is the bond angle:

::: {.math .notranslate .nohighlight}
\\\[cos\\left(\\theta\^{\\alpha\\beta\\gamma}\\right)=\\frac{\\mathbf{r}\^{\\alpha\\beta} \\cdot \\mathbf{r}\^{\\alpha\\gamma}}{r\^{\\alpha\\beta}r\^{\\alpha\\gamma}}\\\]
:::

[\\(S\^{\\alpha\\beta}\\)]{.math .notranslate .nohighlight} is defined as [[(Baskes)]{.std .std-ref}](#baskes97){.reference .internal}:

::: {.math .notranslate .nohighlight}
\\\[\\begin{split}X\^{\\gamma\\beta} = \\left(\\frac{r\^{\\gamma\\beta}}{r\^{\\alpha\\beta}}\\right)\^2\\\\ \\\\ X\^{\\alpha\\gamma} = \\left(\\frac{r\^{\\alpha\\gamma}}{r\^{\\alpha\\beta}}\\right)\^2\\\\ \\\\ C = \\frac{2\\left(X\^{\\alpha\\gamma}+X\^{\\gamma\\beta}\\right)-\\left(X\^{\\alpha\\gamma}-X\^{\\gamma\\beta}\\right)\^2-1}{1-\\left(X\^{\\alpha\\gamma}-X\^{\\gamma\\beta}\\right)\^2}\\\\ \\\\ f_c(x) = \\left\[\\begin{array}{l} 1 \\; \\: x \\geq 1\\\\ \\left(1-\\left(1-x\\right)\^4\\right)\^2 \\; \\: 0\<x\<1\\\\0\\; \\; x\\leq0\\end{array}\\right.\\\\ \\\\ S\^{\\alpha\\beta\\gamma} = f_c\\left(\\frac{C-C\_{min}}{C\_{max}-C\_{min}}\\right)\\\\ \\\\ S\^{\\alpha\\beta} = \\prod\_\\gamma S\^{\\alpha\\beta\\gamma}\\\\\\end{split}\\\]
:::
...
::: {.math .notranslate .nohighlight}
\\\[{}\^r\\!S\\!f_i\^\\alpha = \\sum\_{\\beta} \\left(\\frac{r\^{\\alpha\\beta}}{r_e}\\right)\^ie\^{-\\delta_i \\frac{r\^{\\alpha\\beta}}{r_e}}f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)\\\]
:::

- **bond**

::: {.math .notranslate .nohighlight}
\\\[{}\^b\\!S\\!f\_{ij}\^\\alpha = \\sum\_{\\beta}\\sum\_{\\gamma} \\left(cos(\\theta\_{\\alpha\\beta\\gamma})\\right)\^ie\^{-\\delta_j \\frac{r\^{\\alpha\\beta}}{r_e}}e\^{-\\delta_j \\frac{r\^{\\alpha\\gamma}}{r_e}}f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\gamma}}{dr}\\right)\\\]
:::

- **radialscreened**

::: {.math .notranslate .nohighlight}
\\\[{}\^{rsc}\\!S\\!f_i\^\\alpha = \\sum\_{\\beta} \\left(\\frac{r\^{\\alpha\\beta}}{r_e}\\right)\^ie\^{-\\delta_i \\frac{r\^{\\alpha\\beta}}{r_e}}S\^{\\alpha\\beta}f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)\\\]
:::

- **bondscreened**

::: {.math .notranslate .nohighlight}
\\\[{}\^{bsc}\\!S\\!f\_{ij}\^\\alpha = \\sum\_{\\beta}\\sum\_{\\gamma} \\left(cos(\\theta\_{\\alpha\\beta\\gamma})\\right)\^ie\^{-\\delta_j \\frac{r\^{\\alpha\\beta}}{r_e}}e\^{-\\delta_j \\frac{r\^{\\alpha\\gamma}}{r_e}}S\^{\\alpha\\beta}S\^{\\alpha\\gamma}f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\gamma}}{dr}\\right)\\\]
:::

- **radialspin**

::: {.math .notranslate .nohighlight}
\\\[{}\^{rsp}\\!S\\!f_i\^\\alpha = \\sum\_{\\beta} \\left(\\frac{r\^{\\alpha\\beta}}{r_e}\\right)\^ie\^{-\\delta_i \\frac{r\^{\\alpha\\beta}}{r_e}}\\left(\\mathbf{s\^\\alpha \\cdot s\^\\beta}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)\\\]
:::

- **bondspin**

::: {.math .notranslate .nohighlight}
\\\[{}\^{bsp}\\!S\\!f\_{ij}\^\\alpha = \\sum\_{\\beta}\\sum\_{\\gamma} \\left(cos(\\theta\_{\\alpha\\beta\\gamma})\\right)\^ie\^{-\\delta_j \\frac{r\^{\\alpha\\beta}}{r_e}}e\^{-\\delta_j \\frac{r\^{\\alpha\\gamma}}{r_e}}\\left(\\mathbf{s\^\\alpha \\cdot s\^\\beta}\\right)\\left(\\mathbf{s\^\\alpha \\cdot s\^\\gamma}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\gamma}}{dr}\\right)\\\]
:::

- **radialscreenedspin**

::: {.math .notranslate .nohighlight}
\\\[{}\^{rscsp}\\!S\\!f_i\^\\alpha = \\sum\_{\\beta} \\left(\\frac{r\^{\\alpha\\beta}}{r_e}\\right)\^ie\^{-\\delta_i \\frac{r\^{\\alpha\\beta}}{r_e}}S\^{\\alpha\\beta}\\left(\\mathbf{s\^\\alpha \\cdot s\^\\beta}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)\\\]
:::

- **bondscreenedspin**

::: {.math .notranslate .nohighlight}
\\\[{}\^{bscsp}\\!S\\!f\_{ij}\^\\alpha = \\sum\_{\\beta}\\sum\_{\\gamma} \\left(cos(\\theta\_{\\alpha\\beta\\gamma})\\right)\^ie\^{-\\delta_j \\frac{r\^{\\alpha\\beta}}{r_e}}e\^{-\\delta_j \\frac{r\^{\\alpha\\gamma}}{r_e}}S\^{\\alpha\\beta}S\^{\\alpha\\gamma}\\left(\\mathbf{s\^\\alpha \\cdot s\^\\beta}\\right)\\left(\\mathbf{s\^\\alpha \\cdot s\^\\gamma}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\beta}}{dr}\\right)f_c\\left(\\frac{r_c-r\^{\\alpha\\gamma}}{dr}\\right)\\\]
:::
...
Pair style *rann* does not support computing per-atom stress or using [[pair_modify nofdotr]{.doc}]pair_modify.md){.reference .internal}.
:::
