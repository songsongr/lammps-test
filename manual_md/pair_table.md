...
::::::::::::::::: {#pair-style-table-command .section}
[]{#index-3}[]{#index-2}[]{#index-1}[]{#index-0}

# pair_style table command[](#pair-style-table-command "Link to this heading"){.headerlink}
...
For the *bitmap* style, the specified *N* is used to create interpolation tables that are 2\^N in length. The distance *R* is used to index into the table via a fast bit-mapping technique due to [[(Wolff)]{.std .std-ref}](#wolff2){.reference .internal}, and a linear interpolation is performed between adjacent table values.
...
If your tabulated potential(s) are designed to be used as the short-range part of one of the long-range solvers specified by the [[kspace_style]{.doc}]kspace_style.md){.reference .internal} command, then you must use one or more of the optional keywords listed above for the pair_style command. These are *ewald* or *pppm* or *msm* or *dispersion* or *tip4p*. This is so LAMMPS can ensure the short-range potential and long-range solver are compatible with each other, as it does for other short-range pair styles, such as [[pair_style lj/cut/coul/long]{.doc}]pair_lj_cut_coul.md){.reference .internal}. Note that it is up to you to ensure the tabulated values for each pair of atom types has the correct functional form to be compatible with the matching long-range solver.
...
- Always use the [[pair_write]{.doc}]pair_write.md){.reference .internal} command to produce a plot of what the final interpolated potential looks like. This can show up interpolation "features" you may not like.
...
Suitable tables in the correct format for use with these pair styles can be created by LAMMPS itself using the [[pair_write]{.doc}]pair_write.md){.reference .internal} command. In combination with the pair styles [[python]{.doc}]pair_python.md){.reference .internal}, [[lepton, or lepton/coul]{.doc}]pair_lepton.md){.reference .internal} this can be a powerful mechanism to implement and test tables for use with LAMMPS. Another option to generate tables is the Python code in the [`tools/tabulate`{.docutils .literal .notranslate}]{.pre} folder of the LAMMPS source code distribution.
...
[[pair_coeff]{.doc}]pair_coeff.md){.reference .internal}, [[pair_write]{.doc}]pair_write.md){.reference .internal}
:::
