...
:::::::::::::::: {#pair-style-table-rx-command .section}
[]{#index-1}[]{#index-0}

# pair_style table/rx command[](#pair-style-table-rx-command "Link to this heading"){.headerlink}
...
Style *table/rx* is used in reaction DPD simulations,where the coarse-grained (CG) particles are composed of *m* species whose reaction rate kinetics are determined from a set of *n* reaction rate equations through the [[fix rx]{.doc}]fix_rx.md){.reference .internal} command. The species of one CG particle can interact with a species in a neighboring CG particle through a site-site interaction potential model. Style *table/rx* creates interpolation tables of length *N* from pair potential and force values listed in a file(s) as a function of distance. The files are read by the [[pair_coeff]{.doc}]pair_coeff.md){.reference .internal} command.
...
For the *bitmap* style, the N means to create interpolation tables that are 2\^N in length. The pair distance is used to index into the table via a fast bit-mapping technique [[(Wolff)]{.std .std-ref}](#wolff){.reference .internal} and a linear interpolation is performed between adjacent table values.
