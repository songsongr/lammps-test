...
::::::::::::::::: {#replicate-command .section}
[]{#index-0}

# replicate command[](#replicate-command "Link to this heading"){.headerlink}
...
When the new simulation box is created it is partitioned into a regular 3d grid of rectangular bricks, one per processor, based on the number of processors being used and the settings of the [[processors]{.doc}]processors.md){.reference .internal} command. The partitioning can be changed by subsequent [[balance]{.doc}]balance.md){.reference .internal} or [[fix balance]{.doc}]fix_balance.md){.reference .internal} commands.
...
The bond discussion which follows only refers to models with permanent covalent bonds typically defined in LAMMPS via a data file. It is not relevant to systems modeled with many-body potentials which can define bonds on-the-fly, based on the current positions of nearby atoms, e.g. models using the [[AIREBO]{.doc}]pair_airebo.md){.reference .internal} or [[ReaxFF]{.doc}]pair_reaxff.md){.reference .internal} potentials.
:::
...
To work around this restriction two options are possible. (1) Fixes which use the stored data in the restart file can be defined before replication and then deleted via the [[unfix]{.doc}]unfix.md){.reference .internal} command and re-defined after it. Or (2) the restart file can be converted to a data file (which deletes the stored fix information) and fixes defined after the replicate command. In both these scenarios, the per-atom fix information in the restart file is lost.
:::
