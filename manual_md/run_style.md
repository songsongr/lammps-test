...
::::::::::::::::::::::::::: {#run-style-command .section}
[]{#index-0}

# run_style command[](#run-style-command "Link to this heading"){.headerlink}
...
The *verlet/split* style is also a velocity-Verlet integrator, but it splits the force calculation within each timestep over 2 partitions of processors. See the [[-partition command-line switch]{.doc}]Run_options.md){.reference .internal} for info on how to run LAMMPS with multiple partitions.

Specifically, this style performs all computation except the [[kspace_style]{.doc}]kspace_style.md){.reference .internal} portion of the force field on the first partition. This include the [[pair style]{.doc}]pair_style.md){.reference .internal}, [[bond style]{.doc}]bond_style.md){.reference .internal}, [[neighbor list building]{.doc}]neighbor.md){.reference .internal}, [[fixes]{.doc}]fix.md){.reference .internal} including time integration, and output. The [[kspace_style]{.doc}]kspace_style.md){.reference .internal} portion of the calculation is performed on the second partition.
...
To use the *verlet/split* style, you must define 2 partitions with the [[-partition command-line switch]{.doc}]Run_options.md){.reference .internal}, where partition P1 is either the same size or an integer multiple of the size of the partition P2. Typically having P1 be 3x larger than P2 is a good choice, since the (serial) performance of LAMMPS is often best if the time spent in the [`Pair`{.docutils .literal .notranslate}]{.pre} computation versus [`Kspace`{.docutils .literal .notranslate}]{.pre} is a 3:1 split. The 3d processor layouts in each partition must overlay in the following sense. If P1 is a Px1 by Py1 by Pz1 grid, and P2 = Px2 by Py2 by Pz2, then Px1 must be an integer multiple of Px2, and similarly for Py1 a multiple of Py2, and Pz1 a multiple of Pz2.

Typically the best way to do this is to let the first partition choose its own optimal layout, then require the second partition's layout to match the integer multiple constraint. See the [[processors]{.doc}]processors.md){.reference .internal} command with its *part* keyword for a way to control this, e.g.
...
You can also use the [[partition]{.doc}]partition.md){.reference .internal} command to explicitly specify the processor layout on each partition. E.g. for 2 partitions of 60 and 15 processors each:
...
When you run in 2-partition mode with the *verlet/split* style, the thermodynamic data for the entire simulation will be output to the log and screen file of the first partition, which are log.lammps.0 and screen.0 by default; see the [[-plog and -pscreen command-line switches]{.doc}]Run_options.md){.reference .internal} to change this. The log and screen file for the second partition will not contain thermodynamic output beyond the first timestep of the run.

See the [[Accelerator packages]{.doc}]Speed_packages.md){.reference .internal} page for performance details of the speed-up offered by the *verlet/split* style. One important performance consideration is the assignment of logical processors in the 2 partitions to the physical cores of a parallel machine. The [[processors]{.doc}]processors.md){.reference .internal} command has options to support this, and strategies are discussed in [[Section 5]{.doc}]Speed.md){.reference .internal} of the manual.

------------------------------------------------------------------------

The *respa* style implements the rRESPA multi-timescale integrator [[(Tuckerman)]{.std .std-ref}](#tuckerman3){.reference .internal} with N hierarchical levels, where level 1 is the innermost loop (shortest timestep) and level N is the outermost loop (largest timestep). The loop factor arguments specify what the looping factor is between levels. N1 specifies the number of iterations of level 1 for a single iteration of level 2, N2 is the iterations of level 2 per iteration of level 3, etc. N-1 looping parameters must be specified.
...
The [[timestep]{.doc}]timestep.md){.reference .internal} command sets the large timestep for the outermost rRESPA level. Thus if the 3 loop factors are "2 2 2" for 4-level rRESPA, and the outer timestep is set to 4.0 fs, then the inner timestep would be 8x smaller or 0.5 fs. All other LAMMPS commands that specify number of timesteps (e.g. [[thermo]{.doc}]thermo.md){.reference .internal} for thermo output every N steps, [[neigh_modify delay/every]{.doc}]neigh_modify.md){.reference .internal} parameters, [[dump]{.doc}]dump.md){.reference .internal} every N steps, etc) refer to the outermost timesteps.
...
For fixes that support it, the rRESPA level at which a given fix is active, can be selected through the [[fix_modify]{.doc}]fix_modify.md){.reference .internal} command.
...
The *respa/omp* style is a variant of *respa* adapted for use with pair, bond, angle, dihedral, improper, or kspace styles with an *omp* suffix. It is functionally equivalent to *respa* but performs additional operations required for managing *omp* styles. For more on *omp* styles see the [[Speed omp]{.doc}]Speed_omp.md){.reference .internal} doc page. Accelerated styles take the same arguments and should produce the same results, except for round-off and precision issues.

You can specify *respa/omp* explicitly in your input script, or you can use the [[-suffix command-line switch]{.doc}]Run_options.md){.reference .internal} when you invoke LAMMPS, or you can use the [[suffix]{.doc}]suffix.md){.reference .internal} command in your input script.
...
The *verlet/split* style can only be used if LAMMPS was built with the REPLICA package. Correspondingly the *respa/omp* style is available only if the OPENMP package was included. See the [[Build package]{.doc}]Build_package.md){.reference .internal} page for more info.
...
[[timestep]{.doc}]timestep.md){.reference .internal}, [[run]{.doc}]run.md){.reference .internal}
:::
