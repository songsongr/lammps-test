...
# shell command[](#shell-command "Link to this heading"){.headerlink}
...
The *cd* command changes the current working directory similar to the [`cd`{.docutils .literal .notranslate}]{.pre} command. All subsequent LAMMPS commands that read/write files will use the new directory. All processors execute this command.

The *mkdir* command creates directories similar to the Unix [`mkdir`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-p`{.docutils .literal .notranslate}]{.pre} command. That is, it will attempt to create the entire path of subdirectories if they do not exist yet.
...
The *rm* command deletes file similar to the Unix [`rm`{.docutils .literal .notranslate}]{.pre} command.

The *rmdir* command deletes directories similar to Unix [`rmdir`{.docutils .literal .notranslate}]{.pre} command. If a directory is not empty, its contents are also removed recursively similar to the Unix [`rm`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-r`{.docutils .literal .notranslate}]{.pre} command.

The *putenv* command defines or updates an environment variable directly. Since this command does not pass through the shell, no shell variable expansion or globbing is performed, only the usual substitution for LAMMPS variables defined with the [[variable]{.doc}]variable.md){.reference .internal} command is performed. The resulting string is then used literally.
