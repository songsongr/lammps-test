...
::::::::::::::::::::::::::: {#special-bonds-command .section}
[]{#index-0}

# special_bonds command[](#special-bonds-command "Link to this heading"){.headerlink}
...
A value of 1.0 means include the full interaction without flagging the pair as a "special pair"; a value of 0.0 means exclude the pair completely from the neighbor list, except for pair styles that require a [[kspace style]{.doc}]kspace_style.md){.reference .internal} and pair styles [[amoeba]{.doc}]pair_amoeba.md){.reference .internal}, [[hippo]{.doc}]pair_amoeba.md){.reference .internal}, [[thole]{.doc}]pair_thole.md){.reference .internal}, [[coul/exclude]{.doc}]pair_coul.md){.reference .internal}, and pair styles that include "coul/dsf" or "coul/wolf".
...
For purposes of computing weighted pairwise interactions, 1-3 and 1-4 interactions are not defined from the list of angles or dihedrals used by the simulation. Rather, they are inferred topologically from the set of bonds specified when the simulation is defined from a data or restart file (see [[read_data]{.doc}]read_data.md){.reference .internal} or [[read_restart]{.doc}]read_restart.md){.reference .internal} commands). Thus the set of 1-2,1-3,1-4 interactions that the weights apply to is the same whether angle and dihedral potentials are computed or not, and remains the same even if bonds are constrained, or turned off, or removed during a simulation.
:::

The two exceptions to this rule are (a) if the *angle* or *dihedral* keywords are set to *yes* (see below), or (b) if the [[delete_bonds]{.doc}]delete_bonds.md){.reference .internal} command is used with the *special* option that re-computes the 1-2,1-3,1-4 topologies after bonds are deleted; see the [[delete_bonds]{.doc}]delete_bonds.md){.reference .internal} command for more details.

The *amber* keyword sets the 3 coefficients to 0.0, 0.0, 0.5 for LJ interactions and to 0.0, 0.0, 0.8333 for Coulombic interactions, which is the default for a commonly used version of the AMBER force field, where the last value is really 5/6. See [[(Cornell)]{.std .std-ref}](#cornell){.reference .internal} for a description of the AMBER force field.

The *charmm* keyword sets the 3 coefficients to 0.0, 0.0, 0.0 for both LJ and Coulombic interactions, which is the default for a commonly used version of the CHARMM force field. Note that in pair styles *lj/charmm/coul/charmm* and *lj/charmm/coul/long* the 1-4 coefficients are defined explicitly, and these pairwise contributions are computed as part of the charmm dihedral style - see the [[pair_coeff]{.doc}]pair_coeff.md){.reference .internal} and [[dihedral_style]{.doc}]dihedral_style.md){.reference .internal} commands for more information. See [[(MacKerell)]{.std .std-ref}](#mackerell){.reference .internal} for a description of the CHARMM force field.

The *dreiding* keyword sets the 3 coefficients to 0.0, 0.0, 1.0 for both LJ and Coulombic interactions, which is the default for the Dreiding force field, as discussed in [[(Mayo)]{.std .std-ref}](#mayo){.reference .internal}.

The *fene* keyword sets the 3 coefficients to 0.0, 1.0, 1.0 for both LJ and Coulombic interactions, which is consistent with a coarse-grained polymer model with [[FENE bonds]{.doc}]bond_fene.md){.reference .internal}. See [[(Kremer)]{.std .std-ref}](#kremer){.reference .internal} for a description of FENE bonds.
...
The *one/five* keyword enable calculation and storage of a list of 1-5 neighbors in the molecular topology for each atom. It is required by some pair styles, such as [[pair_style amoeba]{.doc}]pair_style.md){.reference .internal} and [[pair_style hippo]{.doc}]pair_style.md){.reference .internal}.
...
[[delete_bonds]{.doc}]delete_bonds.md){.reference .internal}, [[fix bond/create]{.doc}]fix_bond_create.md){.reference .internal}
:::
