...
::::::::::::::: {#units-command .section}
[]{#index-0}

# units command[](#units-command "Link to this heading"){.headerlink}
...
Potential or table files may have a [`UNITS:`{.docutils .literal .notranslate}]{.pre} tag included in the first line indicating the unit style those files were created for. If the tag exists, its value will be compared to the chosen unit style and LAMMPS will stop with an error message if there is a mismatch. In some select cases and for specific combinations of unit styles, LAMMPS is capable of automatically converting potential parameters from a file. In those cases, a warning message signaling that an automatic conversion has happened is printed to the screen.

------------------------------------------------------------------------

For style *lj*, all quantities are unitless. Without loss of generality, LAMMPS sets the fundamental quantities mass, [\\(\\sigma\\)]{.math .notranslate .nohighlight}, [\\(\\epsilon\\)]{.math .notranslate .nohighlight}, and the Boltzmann constant [\\(k_B = 1\\)]{.math .notranslate .nohighlight}. The masses, distances, energies you specify are multiples of these fundamental values. The formulas relating the reduced or unitless quantity (with an asterisk) to the same quantity with units is also given. Thus you can use the mass, [\\(\\sigma\\)]{.math .notranslate .nohighlight}, and [\\(\\epsilon\\)]{.math .notranslate .nohighlight} values for a specific material and convert the results from a unitless LJ simulation into physical quantities. Please note that using these three properties as base, your unit of time has to conform to the relation [\\(\\epsilon = \\frac{m \\sigma\^2}{\\tau\^2}\\)]{.math .notranslate .nohighlight} since energy is a derived unit (in SI units you equivalently have the relation [\\(1\\mathsf{J} = 1\\frac{\\mathsf{kg}\\cdot\\mathsf{m}\^2}{\\mathsf{s}\^2}\\)]{.math .notranslate .nohighlight}).

- mass = mass or [\\(m\\)]{.math .notranslate .nohighlight}, where [\\(M\^\* = \\frac{M}{m}\\)]{.math .notranslate .nohighlight}

- distance = [\\(\\sigma\\)]{.math .notranslate .nohighlight}, where [\\(x\^\* = \\frac{x}{\\sigma}\\)]{.math .notranslate .nohighlight}

- time = [\\(\\tau\\)]{.math .notranslate .nohighlight}, where [\\(\\tau\^\* = \\tau \\sqrt{\\frac{\\epsilon}{m \\sigma\^2}}\\)]{.math .notranslate .nohighlight}

- energy = [\\(\\epsilon\\)]{.math .notranslate .nohighlight}, where [\\(E\^\* = \\frac{E}{\\epsilon}\\)]{.math .notranslate .nohighlight}

- velocity = [\\(\\frac{\\sigma}{\\tau}\\)]{.math .notranslate .nohighlight}, where [\\(v\^\* = v \\frac{\\tau}{\\sigma}\\)]{.math .notranslate .nohighlight}

- force = [\\(\\frac{\\epsilon}{\\sigma}\\)]{.math .notranslate .nohighlight}, where [\\(f\^\* = f \\frac{\\sigma}{\\epsilon}\\)]{.math .notranslate .nohighlight}

- torque = [\\(\\epsilon\\)]{.math .notranslate .nohighlight}, where [\\(t\^\* = \\frac{t}{\\epsilon}\\)]{.math .notranslate .nohighlight}

- temperature = reduced LJ temperature, where [\\(T\^\* = \\frac{T k_B}{\\epsilon}\\)]{.math .notranslate .nohighlight}

- pressure = reduced LJ pressure, where [\\(p\^\* = p \\frac{\\sigma\^3}{\\epsilon}\\)]{.math .notranslate .nohighlight}

- dynamic viscosity = reduced LJ viscosity, where [\\(\\eta\^\* = \\eta \\frac{\\sigma\^3}{\\epsilon\\tau}\\)]{.math .notranslate .nohighlight}

- charge = reduced LJ charge, where [\\(q\^\* = q \\frac{1}{\\sqrt{4 \\pi \\varepsilon_0 \\sigma \\epsilon}}\\)]{.math .notranslate .nohighlight}

- dipole = reduced LJ dipole, moment where [\\(\\mu\^\* = \\mu \\frac{1}{\\sqrt{4 \\pi \\varepsilon_0 \\sigma\^3 \\epsilon}}\\)]{.math .notranslate .nohighlight}

- electric field = force/charge, where [\\(E\^\* = E \\frac{\\sqrt{4 \\pi \\varepsilon_0 \\sigma \\epsilon} \\sigma}{\\epsilon}\\)]{.math .notranslate .nohighlight}

- density = mass/volume, where [\\(\\rho\^\* = \\rho \\frac{\\sigma\^{dim}}{m}\\)]{.math .notranslate .nohighlight}

Note that for LJ units, the default mode of thermodynamic output via the [[thermo_style]{.doc}]thermo_style.md){.reference .internal} command is to normalize all extensive quantities by the number of atoms. E.g. potential energy is extensive because it is summed over atoms, so it is output as energy/atom. Temperature is intensive since it is already normalized by the number of atoms, so it is output as-is. This behavior can be changed via the [[thermo_modify norm]{.doc}]thermo_modify.md){.reference .internal} command.
...
- For style *lj* these are dt = 0.005 [\\(\\tau\\)]{.math .notranslate .nohighlight} and skin = 0.3 [\\(\\sigma\\)]{.math .notranslate .nohighlight}.
